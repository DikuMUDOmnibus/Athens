...
const flag_type *affect_get_bitvector_table_for_where(int where)
{
	...
	//case WHERE_OBJEXTRA3:   return objextra3_flags; // object
...
void affect_remove_obj( OBJ_DATA *obj, AFFECT_DATA *paf)
{
		...
		switch( paf->where)
	{
		...
		case WHERE_OBJEXTRA2:
            ...
            break;
                /*case WHERE_OBJEXTRA3:
            REMOVE_BIT(obj->extra3_flags,paf->bitvector);
            break;*/
		case WHERE_WEAPON:
            if(obj->item_type == ITEM_WEAPON)
...
bool can_see( char_data *looker, char_data *victim )
{
   ...
   //start add here    
   //implemented so arena gladiators will 'see', fight, and get it over with. 
   if(IS_SET(looker->act2, ACT2_PLR_ARENA) 
      && IS_SET(victim->act2, ACT2_PLR_ARENA)){ 
      return true; 
   } 

   //a 'PC seeall' originally implemented to support arena do_accept. 
    if(IS_SET(looker->act2, ACT2_PLR_CANSEE) 
      && !IS_IMMORTAL(victim)){ 
      return true; 
   } 
   //end add here 
	...
	case APPLY_NONE:
		{
			...
			case WHERE_OBJEXTRA2:
				...
				break;
			/*case WHERE_OBJEXTRA3:
                                sprintf( result,"%s (object)", extra3_bit_name(paf->bitvector));
                                break;*/
			case WHERE_AFFECTS:
			default:
...
/**************************************************************************/
/*char *extra3_bit_name( int bits)
{
        return flag_string(objextra3_flags, bits);
}*/
/**************************************************************************/
...
/**************************************************************************/
char *job_bit_name(int bits)
{
	return flag_string(job_flags, bits);
}        
/**************************************************************************/
...
/**************************************************************************/
// appends to file short time code, room, players name
void append_event_to_file( char_data *ch, char *file, char *str )
{
    char logbuf[MSL];
...
char *to_affect_string( AFFECT_DATA *paf, int objects_level )
{
	...
	case WHERE_OBJEXTRA2:
		...
		break;
	/*case WHERE_OBJEXTRA3:
                sprintf( buf, "Adds object extra3 flag(s) %s.",
