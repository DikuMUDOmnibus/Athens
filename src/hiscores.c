...
/* Hiscoset command */
{
	...
	if( argument[0] == '\0' )
	{
		ch->printlnf("Syntax: hiscoset <command> [table keyword] [field] [value]\n\r", ch );
		ch->printlnf("\n\r", ch );
		ch->printlnf("    Command being one of:\n\r", ch );
		ch->printlnf("    list                     Lists all hiscore tables.\n\r", ch );
		ch->printlnf("    show <kwd>               Shows the data for that table.\n\r", ch );
		ch->printlnf("    create <kwd>             Create new table with that keyword.\n\r", ch );
		ch->printlnf("    destroy <kwd>            Destroy table with that keyword.\n\r", ch );
		ch->printlnf("    set <kwd> <field> <val>  Sets data for that table.\n\r", ch );
		ch->printlnf("\n\r", ch );
		ch->printlnf("    Field being one of:\n\r", ch );
		ch->printlnf("    keyword <str>            Set new keyword for the table.\n\r", ch );
		ch->printlnf("    vnum <num>               Set an object vnum for this table.\n\r", ch );
		ch->printlnf("    desc <str>               Set the description.\n\r", ch );
		ch->printlnf("    maxlength <num>          Set Max entries in the table.\n\r", ch );
		ch->printlnf("    entry <name> <score>     Change the score for a player.\n\r", ch );
		ch->printlnf("\n\r", ch );
		ch->printlnf("    See HELP HISCOSET for more details.\n\r", ch );
		return;
	}
	...
	if( !str_cmp( cmd, "list" ) )
	{
		ch->printlnf( "List of hiscore tables: \n\r", ch );
	...
	if( !str_cmp( cmd, "create" ) && (cmd_found=true) && kwd[0] != '\0' )
	{
		if( table != NULL )
		{
			ch->printlnf( "A table with that keyword already exists.\n\r", ch );
			return;
		}
	...
	if( kwd[0] == '\0' )
	{
		ch->printlnf( "Please specify a table keyword.\n\r", ch );
		return;
	}
	if( table == NULL )
	{
	...
	else if( !str_cmp( cmd, "vnum" ) && (cmd_found=true) && set_val[0] != '\0' )
	{
		if( is_number( set_val) )
		{
			...
			ch->printlnf( "Argument for set vnum must be numeric.\n\r", ch );
		return;
	}
	else if( !str_cmp( cmd, "maxlength" ) && (cmd_found=true) && set_val[0] != '\0' )
	{
		if( is_number( set_val) )
		{
			...
			ch->printlnf( "Argument for set maxlength must be numeric.\n\r", ch );
		return;
	}
	...
	{
		ch->printlnf( "Second argument to set entry must be numberic.\n\r", ch );
		return;
	}
...
/* Hiscore command */
{
	...
	{
		some_table = false;
		ch->printlnf( "Hiscore tables in this game:\n\r", ch );
		for( table = first_table ; table ; table = table->next )
		{
			if( table->vnum == -1 )
			{
	...
	if( table == NULL )
	{
		ch->printlnf( "No such hiscore table.\n\r", ch );
		return;
	}

 	if( table->vnum != -1 && !IS_IMMORTAL(ch) )
	{
		ch->printlnf( "That hiscore table is attached to an object. Go see the object.\n\r", ch );
		return;
	}
