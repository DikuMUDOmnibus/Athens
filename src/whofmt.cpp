...
#include "channels.h"
/***************************************************************************/
/*whoformat_Athens is modified from whoformat_dawn. Gives everything Dawn  */
/*does, but no clss displayed unless wch is an Immortal or Newbie Support  */
/*or IS_NEWBIE_SUPPORT(ch) || IS_IMMORTAL(ch) && IS_NEWBIE(wch). It also   */
/*will print the area wch is in if ch->level >= 45 && < 91, or if ch->level*/
/*>= 91 prints wch at room in area.					   */	
/*	             							   */
/*	**These modifications made by Zeus of Athens Mud**                 */
/***************************************************************************/
char *whoformat_athens ( char_data *ch, char_data *wch, bool two_column)
{
    ...
    {
		default: clss="***";break;
		case MAX_LEVEL - 0 : clss = "`#`R[`^   IMP   `R]`^";     break;
                case MAX_LEVEL - 1 : clss = "`#`R[`^DEPT HEAD`R]`^";     break;
                case MAX_LEVEL - 2 : clss = "`#`R[`^SR  ADMIN`R]`^";     break;
                case MAX_LEVEL - 3 : clss = "`#`R[`^JR  ADMIN`R]`^";     break;
                case MAX_LEVEL - 4 : clss = "`#`R[`^  CODER  `R]`^";     break;
                case MAX_LEVEL - 5 : clss = "`#`R[`^ BUILDER `R]`^";     break;
                case MAX_LEVEL - 6 : clss = "`#`R[`^PROBATION`R]`^";     break;
                case MAX_LEVEL - 7 : clss = "`#`R[`^   NSA   `R]`^";     break;
                case MAX_LEVEL - 8 : clss = "`#`R[`^///TBD///`R]`^";     break;

	}
		
		if(!str_cmp(wch->name, "eris"))
		{clss="`#`R[`Y~`WHD BLDR`Y~`R]`^";}

		if(!str_cmp(wch->name, "dionysus"))
		{clss="`#`R[`^`WCRAZY-MAN`R]`^";}

		if(!str_cmp(wch->name, "archiel"))
		{clss="`#`R[`^`W  GUEST  `R]`^";}
		
		if(!str_cmp(wch->name, "zeus") || !str_cmp(wch->name, "hera") && wch->level >= 92)
		{clss="`#`R[`^  `WOWNER  `R]`^";}
	...
	else if (IS_NEWBIE_SUPPORT(wch) && IS_NEWBIE(ch))
		{
		clss="`#`R[`CNSP`R]`^";
		}
	 else if (IS_NEWBIE_SUPPORT(wch) && IS_IMMORTAL(ch))
		{
		clss="`#`R[`CNSP`R]`^";
                }
	else if (IS_NEWBIE_SUPPORT(wch) && IS_NEWBIE_SUPPORT(ch))
		{
		clss="`#`R[`CNSP`R]`^";		}
 ...
 // Format it up //%[%*%]#  %new%i%w%b%a%o%O%P%i%h%s%Q%C%l%o%q
	sprintf( buf, "%s%s%s`^%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s",
		...
		IS_SET(wch->comm, COMM_CODING) ? " [`#`RCode`^]" : "",
...
sprintf( buf, "%s%s%s`^%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s",
		//"%s%s%s%s%s
		clss,
		((IS_IMMORTAL(ch)||HAS_IMMTALK_NAME(ch))? (HAS_IMMTALK_NAME(wch)?"`#`=\x92#`&":" ") :""),
        name,
		...
		incogtext,
		wizitext,
        IS_SET(wch->comm, COMM_BUILDING) ? " [`#`BBuilding`^]" : "",
        IS_SET(wch->comm, COMM_CODING) ? " [`#`RCoding`^]" : "",
	IS_SET(wch->comm, COMM_BUSY) ? " [`#`yBusy`^]" : "",
        IS_SET(wch->comm, COMM_AFK) ? " `#`=\x84[AFK]`^" : "",
         (!IS_IMMORTAL(wch) && IS_OOC(wch))? " `#`=^(in OOC)`^" : "",
...
/* Display wch area/room name if ch level > 45/91. Do not display Immortal area/room */	
if(ch->level >= 91 && wch->in_room && wch->level<LEVEL_IMMORTAL
	&& wch->in_room->area && wch->in_room->name && wch->in_room->area->name)
	{
        sprintf(buf,"`xat %s in %s", wch->in_room->name, wch->in_room->area->name);
        strcat(return_buf,buf);
    	}

else if(ch->level >= 45 && ch->level < 91 && wch->level<LEVEL_IMMORTAL && wch->in_room->area && wch->in_room->name && wch->in_room->area->name)
	{
	sprintf(buf,"`xin %s", wch->in_room->area->name);
	strcat(return_buf,buf);
	}
...
char *whoformat_dawn ( char_data *ch, char_data *wch, bool two_column)
{
    ...
    switch ( wch->level )
    {
		default: clss="***";break;
                ...
                case MAX_LEVEL - 1 : clss = "DEPT HEAD";     break;
                case MAX_LEVEL - 2 : clss = "SR  ADMIN";     break;
                case MAX_LEVEL - 3 : clss = "JR  ADMIN";     break;
                case MAX_LEVEL - 4 : clss = "  CODER  ";     break;
                case MAX_LEVEL - 5 : clss = " BUILDER ";     break;
                case MAX_LEVEL - 6 : clss = "PROBATION";     break;
                case MAX_LEVEL - 7 : clss = "   NSA   ";     break;
                case MAX_LEVEL - 8 : clss = "///TBD///";     break;

	}
		if (!str_cmp(wch->name, "zeus") && wch->level >= 92){clss 
= "Head Admin";}
		if (!str_cmp(wch->name, "hera") && wch->level >=92){clss = "  OWNER  ";}
...
//	if(HAS_CONFIG2(wch,CONFIG2_GUEST)){
	...
	if IS_SET(wch->pcdata->council, COUNCIL_HEADSUPPORT)  strcat(w3,"`WS");    // S
	...
	else if IS_SET(wch->pcdata->council, COUNCIL_REMORTS)  strcat(w3,"`RB");        // B
	else strcat(w3, "`R-");
...
char *whoformat_endless( char_data *ch, char_data *wch, bool two_column)
{
	...
	else if IS_SET(wch->pcdata->council, COUNCIL_REMORTS)  strcat(w2,"`RB");        // B
...
//		else 
    ...
    switch ( wch->level )
    {
		...
		{clss = "HEAD ADMIN"; break;}
		case MAX_LEVEL + 4 : clss = " FOUNDER ";     break;
		case MAX_LEVEL - 0 : clss = "   IMP   ";     break;
		case MAX_LEVEL - 1 : clss = "DEPT HEAD";     break;
		case MAX_LEVEL - 2 : clss = "SR  ADMIN";     break;
		case MAX_LEVEL - 3 : clss = "JR  ADMIN";     break;
	...
	if( wch->level<LEVEL_IMMORTAL)
	{
		...
		if (IS_NEWBIE_SUPPORT(wch) && IS_NEWBIE_SUPPORT(ch))
			clss = "NST";
		else {clss = class_table[wch->clss].short_name;
                        sprintf(whoclass,"`#`G%s`^", clss);
                        clss=whoclass;}
	}
