...
SPRESULT spell_holy_aura( int sn, int level, char_data *ch, void *vo, int  )
{
	...
	if(!is_affected(victim,sn))
	{
		...
		affect_to_char( victim, &af);
		victim->println("You feel your goodness protecting you.");
		act("$n is protected by goodness.", ch, NULL, victim, TO_NOTVICT);
	}
	else
	{
		if(ch == victim)
			ch->println("You are already protected by goodness.");
		else
...
SPRESULT spell_magic_resistance( int sn, int level, char_data *, void *vo, int )
{
	...
	if(!is_affected(victim,sn))
	{
		...
		affect_to_char(victim, &af);
		victim->println("You are protected from mysterious powers.");
	}
	else
	{
		victim->println("You are already protected from mysterious powers.");
		return HALF_MANA;
	}
	return FULL_MANA;
}
...
SPRESULT spell_protection_fire( int sn, int level, char_data *, void *vo, int  )
{
	...
	if(!is_affected(victim,sn))
	{
		...
		af.bitvector	= RES_FIRE;
		affect_to_char(victim, &af);
		victim->println("You feel the protection of Hephaestus upon you.");
	}
	else
	{
		victim->println("You are already protected by Hephaestus from fire.");
		return HALF_MANA;
	}
	return FULL_MANA;
}
...
SPRESULT spell_resist_poison( int sn, int level, char_data *, void *vo, int )
{
	...
	if(!is_affected(victim,sn))
	{
		...
		affect_to_char(victim, &af);
		victim->println("Using the skills taught by Asclepius, you protect yourself.");
	}
	else
	{
		victim->println("You are already protected from poison as Asclepius has taught men to do.");
		return HALF_MANA;
	}
