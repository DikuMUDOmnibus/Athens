		...
		case 'P': // put an object inside another object (in the room)
			...
			if (!obj_to)
			{
				...
				if(GAMESETTING3(GAMESET3_DISPLAY_P_RESET_BUGS_ON_WIZNET)){
					wiznet(buf2,NULL,NULL,WIZ_BUGS,0,CODER); // put it on the bug channel 
				}
				log_string( buf2 );
...
char *get_areaname(char_data *ch, AREA_DATA *pArea, bool longformat)
{
	...
	if(longformat){
		...
		if (IS_SET(pArea->area_flags, AREA_OLCONLY)){
			if(c_str_len(aname)>30){
				...
				sprintf(areaname,"%s", str_width(aname,16));
			}
		}else{
			sprintf(areaname,"%s`x",str_width(aname,16));
		}
	}else{
	...
	if(longformat){
		sprintf( buf, "%s%s `^%s`x",
			areaname,
			(IS_NULLSTR(pArea->credits)?str_width(pArea->builders,40):str_width(pArea->credits, 40)),
			str_width(lrange, 12));
	}else{
		sprintf( buf, "%s %s`x ",
			areaname,
	...
	for ( pArea = area_arealist_first; pArea; pArea = pArea->arealist_sort_next)
    {
		...
		if (ch->lines && linenum%ch->lines==0 || linenum==0) // print header once every page 
		{
			if(IS_IMMORTAL(ch)){ // mapscale info
				...
				add_buf( output, "`CArea Name       Credits                                  Area Comment`x\r\n");
			}
/*			if(linenum==0){ 
				if(ch->in_room && ch->in_room->area){
					// hidden areas aren't seen by morts on the arealist				
					if (!IS_SET(ch->in_room->area->area_flags, AREA_HIDDEN)||IS_IMMORTAL(ch)){
						char wbuf[MIL];
						sprintf(wbuf,"`cYou are currently in %s`x", get_areaname(ch, ch->in_room->area, false));
						add_buf( output, wbuf);
					}else{
						add_buf( output, "`cYou are currently in an unknown area.`x\r\n");
					}
				}else{
					add_buf( output, "`cYou are currently in an unknown area!.`x\r\n");
				}
