...
void acid_effect(void *vo, int level, int dam, int target)
{
    ...
    if (target == TARGET_OBJ) // toast an object 
    {
		...
		case ITEM_CORPSE_NPC:
			msg = "`c$p fumes and dissolves.`x";
			break;
		case ITEM_ARMOR:
			msg = "`c$p is pitted and etched.`x";
			break;
		case ITEM_CLOTHING:
			msg = "`c$p is corroded into scrap.`x";
			break;
		case ITEM_STAFF:
		case ITEM_WAND:
			chance -= 10;
			msg = "`c$p corrodes and breaks.`x";
			break;
		case ITEM_SCROLL:
			chance += 10;
			msg = "`c$p is burned into waste`x.";
			break;
		case ITEM_COMPONENT:
			chance -= 10;
			msg = "`c$p melts before your very eyes.`x";
			break;
		}
...
void fire_effect(void *vo, int level, int dam, int target)
{
    ...
    if (target == TARGET_CHAR)   /* do the effect on a victim */
    {
		...
		{
            AFFECT_DATA af;
            act("`c$n is blinded by smoke!`x",victim,NULL,NULL,TO_ROOM);
            act("`cYour eyes tear up from smoke...you can't see a thing!`x",
    ...
    if (target == TARGET_OBJ)  /* toast an object */
    {
        ...
        default:             
			return;
        case ITEM_CONTAINER:
            msg = "`c$p ignites and burns!`x";
            break;
        case ITEM_POTION:
		case ITEM_CAULDRON:
            chance += 25;
            msg = "`c$p bubbles and boils!`x";
            break;
        case ITEM_SCROLL:
            chance += 50;
            msg = "`c$p crackles and burns!`x";
            break;
        case ITEM_STAFF:
            chance += 10;
            msg = "`c$p smokes and chars!`x";
            break;
        case ITEM_WAND:
            msg = "`c$p sparks and sputters!`x";
            break;
        case ITEM_FOOD:
            msg = "`c$p blackens and crisps!`x";
            break;
        case ITEM_PILL:
            msg = "`c$p melts and drips!`x";
            break;
		case ITEM_COMPONENT:
			chance -= 10;
			msg = "`c$p is consumed by fire.`x";
			break;
...
void poison_effect(void *vo,int level, int dam, int target)
{
    ...
    if (target == TARGET_CHAR)   /* do the effect on a victim */
    {
			...
			&& HAS_CLASSFLAG(victim, CLASSFLAG_POISON_IMMUNITY))
        {
            ...
            victim->println("`cYou feel suddenly ill.`x");
...
void shock_effect(void *vo,int level, int dam, int target)
{
    ...
    if (target == TARGET_CHAR)
    {
		...
		{
			victim->println("`cYour muscles stop responding.`x");
    ...
    if (target == TARGET_OBJ)
    {
		...
		case ITEM_STAFF:
			chance += 10;
			msg = "`c$p overloads and explodes!`x";
			break;
		case ITEM_JEWELRY:
			chance -= 10;
			msg = "`c$p is fused into a worthless lump.`x";
		}
