...
void do_mset( char_data *ch, char *argument )
{
    ...
    {
		ch->printlnf("`CSilver`x changed from %ld to %d on %s(%s)",
			victim->silver, value, victim->short_descr, victim->name);
		victim->silver = value;
		return;
    }
    if ( !str_prefix(arg2, "fame" ) )
    {
		ch->printlnf("`MFame`x changed from %ld to %d on %s(%s)",
			victim->pcdata->fame, value, victim->short_descr, victim->name);
		victim->pcdata->fame = value;
		return;
    }
    if ( !str_prefix(arg2, "mps" ) )
    {
		ch->printlnf("`MMilitary Points`x changed from %ld to %d on %s(%s)",
			victim->pcdata->military_points, value, victim->short_descr, victim->name);
		victim->pcdata->military_points = value;
		return;
    }
    if ( !str_prefix(arg2, "clout" ) )
    {
		ch->printlnf("`MClout`x changed from %ld to %d on %s(%s)",
			victim->pcdata->clout, value, victim->short_descr, victim->name);
		victim->pcdata->clout = value;
		return;
    }
    if ( !str_prefix(arg2, "divinity" ) )
    {
		ch->printlnf("`MDivinity`x changed from %ld to %d on %s(%s)",
			victim->pcdata->divinity, value, victim->short_descr, victim->name);
...
void do_force( char_data *ch, char *argument )
{
		...
		for ( vch = player_list; vch; vch = vch_next_player )
		{
			...
			if ( !IS_NPC(vch) && get_trust( vch ) < get_trust( ch ) )
			{
				if (can_see_who(vch, TRUE_CH(ch))){
					...
					vch->printlnf("An Olympian god forces you to '%s'.", argument);
				}
    ...
    else
    {
		...
		if (can_see_who(victim, TRUE_CH(ch))){
			...
			victim->printlnf("An Olympian god forces you to '%s'.", argument);
		}
...
/**************************************************************************/
void do_holyset(char_data *ch, char *argument)
{
  char arg[MIL];
  argument = one_argument(argument, arg);
  ...
  if (!str_prefix(arg, "\0"))
    {
      ch->printlnf("Say what? Try Syntax: HOLYSET <type>");
      ch->printlnf(" Type: LIGHT, SPEECH, NAME, WALK, or VNUM");
      return;
    }

  if (!str_prefix(arg, "speech"))
  {
    if(IS_UNSWITCHED_MOB(ch))
      {
	    return;
...
void do_whoinvis( char_data *ch, char *)
{
    ...
    {
		REMOVE_BIT(TRUE_CH(ch)->comm, COMM_WHOVIS);
		ch->printlnf( "You are now `Rinvisible`X on the wholist.");
   }
    else
    {
      SET_BIT(TRUE_CH(ch)->comm, COMM_WHOVIS);
	      ch->printlnf("you are now `Cvisible`X on the wholist.");
    }
    return;
}
...
void do_rename (char_data* ch, char* argument)
{
    ...
    victim->wraplnf("You have been renamed to '%s' by %s.",
		victim->name, can_see_who(victim, ch)?ch->name:"an Olympian god");
...
void do_noterestrict( char_data *ch, char *argument )
{
    ...
    if (!IS_ADMIN(ch))
        //&& !IS_SET(TRUE_CH(ch)->pcdata->council, COUNCIL_HEADLAW))	
    {
        ch->println( "Noterestrict can only be used by an admin." );
        return;
    }
	one_argument( argument, arg );
...
void do_strip_affects( char_data *ch, char *argument )
{
	...
	if (( !str_cmp( arg2, "all" )) || ( IS_NULLSTR ( arg2 ))) {
		...
		if ( str_cmp( victim->name, ch->name ))
			victim->println( "Any affects upon you from the gods have been stripped away!" );
	...
	if ( is_affected( victim, sn )) {
		...
		if ( str_cmp( victim->name, ch->name ))
			victim->println("Some of your affects have been stripped away!" );
