...
char * substitute_alias(connection_data *d, char *argument)
{
	...
	for (alias = 0; alias < MAX_ALIAS; alias++)	 // go through the aliases 
	{
		...
		if (!str_prefix(ch->pcdata->alias[alias],argument))
		{
			...
			{
				ch->println("`cAlias substitution too long. Truncated.`x");
				buf[MIL -1] = '\0';
			}
			break;
		}
...
{
	ch->println("`cI'm sorry, alias must be entered in full.`x");
	return;
}
...
void do_alias(char_data *ch, char *argument)
{
	...
	if (arg[0] == '\0')
	{
		...
		{
			ch->println("`cYou have no aliases defined.`x");
			return;
		}
		ch->printlnbw( "`cYour current aliases are:`x" );
	...
	{
		ch->println("`cSorry, that word is reserved.`x");
		return;
	}
	...
	if (argument[0] == '\0')
	{
		for (pos = 0; pos < MAX_ALIAS; pos++)
		{
			...
			{
				sprintf(buf,"`c%s aliases to '%s'.`x",
		...
		ch->println("`cThat alias is not defined.`x");
		return;
	}
	
	if (!str_prefix(argument,"delete") || !str_prefix(argument,"prefix"))
	{
		ch->println("`cThat shall not be done!`x");
		return;
	}
	...
	for (pos = 0; pos < MAX_ALIAS; pos++)
	{
		...
		if (!str_cmp(arg,rch->pcdata->alias[pos])) /* redefine an alias */
		{
			...
			sprintf(buf,"`c%s is now re-aliased to '%s'.`x",arg,argument);
			ch->printlnbw(buf);
			return;
		}
	}
	...
	if (pos >= MAX_ALIAS)
	{
		ch->println("`cSorry, you have reached the alias limit.`x");
		return;
	}
	...
	rch->pcdata->alias_sub[pos]	= str_dup(argument);
	ch->printlnf( "`c%s is now aliased to '%s'.`x",	arg, argument);
}
...
void do_unalias(char_data *ch, char *argument)
{
	...
	if (!found)
		ch->println("`cNo alias of that name to remove.`x");
	return;
}
