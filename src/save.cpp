...
void fwrite_char( char_data *ch, FILE *fp )
{
	...
	if(!IS_NPC(ch))
	{
		...
		fprintf( fp, "Fame  %ld\n", ch->pcdata->fame );
		fprintf( fp, "Military_points  %ld\n", ch->pcdata->military_points );
		fprintf( fp, "Clout  %ld\n", ch->pcdata->clout );
		fprintf( fp, "Divinity  %ld\n", ch->pcdata->divinity );
	...
	if(ch->job)
	{
		fwrite_wordflag(job_flags, ch->job, "Job",fp);
	}
	...
	for ( ; ; )
	{
		...
		case 'C':
			KEY( "Clan",		ch->clan,						clan_slookup(fread_string(fp)));
                  	KEY("Clout", ch->pcdata->clout, fread_number (fp));
		...
		case 'J':
			KEY ( "JOB",	ch->job,		fread_wordflag( job_flags, fp));
...
KEY( "Lost",         ch->lost,            fread_number( fp ) );
		...
		case 'M':
			...
			KEY( "MiscN", ch->pcdata->last_misc,	fread_number( fp ));
                  	KEY("Military_points", ch->pcdata->military_points, fread_number(fp));
			break;
...
obj_data * fread_obj( FILE *fp, const char *filename )
{
    ...
    for ( ; ; )
    {
		...
		case 'E':
			...
			//KEY( "ExtF3",       obj->extra3_flags,       fread_number( fp ) );
