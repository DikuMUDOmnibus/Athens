...
void do_raceinfo( char_data *ch, char *argument )
{
	...
	if(IS_IMMORTAL(ch)){
		ch->println("`xThe game currently has the following citizenships (`Y*`x=not creation selectable):");
	}else{
		ch->println("`xThe game currently has the following citizenships:");
	}

	// create and display a titlebar
	sprintf(titlebar, "%s===`YCitizenship`m===`Yst`m==`Yqu`m==`Ypr`m==`Yem`m==`Yin`m==`Yco`m==`Yag"
		"`m==`Ysd`m==`Yme`m==`Yre`m=`Ytotal`m==`Ycp`m===`Yhp`m==`x",
		(GAMESETTING(GAMESET_REMORT_SUPPORTED)?"`Yremort`m":"`m====="));	
	ch->println(titlebar);
	...
	for( race = 1; race_table[race]; race++ )
	{
		...
		if(GAMESETTING(GAMESET_REMORT_SUPPORTED)){
			sprintf(buf,"`Y%d",race_table[race]->remort_number);			
			strcat(linebuf,buf);
		}
