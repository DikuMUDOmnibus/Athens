...
void arena_broadcast(char_data *ch, char * fmt, ...); //Arena handle

char *  const   death_cry_dir_name[]=
{
    "`cthe north`x", "`cthe east`x", "`cthe south`x", "`cthe west`x", "`cabove`x", "`cbelow`x", 
	"`cthe northeast`x", "`cthe southeast`x", "`cthe southwest`x", "`cthe northwest`x"
};
...
void kill_char(char_data *victim, char_data *ch)
{
	...
	if ( !IS_NPC(victim) )
	{
		if (IS_NPC(ch)){ // mob killed player
			...
			// send the info
			info_broadcast(victim, "%s has been slain by %s.",	victim->name, ch->short_descr);
			// Log mobkills
			{
				char mklogbuf[MSL];
         ...
         //Arena-use 
      }else if(IS_SET(ch->act2, ACT2_PLR_ARENA) 
            || IS_SET(victim->act2, ACT2_PLR_ARENA)){ 
            char buf[MIL]; 
            sprintf( buf, "%s has been defeated by %s", victim->name, ch->name );    
            arena_broadcast( ch, buf ); 
            ch->win += 1; 
            victim->lost += 1; //arena stats 
            char_data *wch; 
            //Arena betting system, pays the winners 
            ...
            { 
               ...
               { 
                  ...
                  if (IS_SET(wch->act2,ACT2_PLR_BET) && IS_SET(victim->act2, ACT2_PLR_ARENA) ) 
                  { 
                     ...
                     wch->gold += value; 
                     wch->pot = 0; 
                     wch->printlnf("`R[ARENA]:`x You won the bet! You receive %d `Ygold.\n\r", value); 
                     wch->pcdata->bet = NULL; 
                     REMOVE_BIT(wch->act2, ACT2_PLR_BET); 
                     } 
                     if (!IS_NPC(wch) && !str_cmp(wch->pcdata->bet,victim->name)) 
                     { 
                     wch->printlnf("`R[ARENA]:`x You lost the %d `Ygold you bet!\n\r", wch->pot); 
                     wch->pot = 0; 
                     wch->pcdata->bet = NULL; 
                     REMOVE_BIT(wch->act2, ACT2_PLR_BET); 
                     } 
			...
			// send the info
			info_broadcast(victim, "%s has been slain by %s!", victim->name, ch->name);
      ...
      && !IS_SET(ch->act2, ACT2_PLR_ARENA) && !IS_SET(victim->act2, ACT2_PLR_ARENA)
//	&& !IS_SET(ch->act2, ACT2_PLR_EVENT) && !IS_SET(victim->act2, ACT2_PLR_EVENT)
	){ 
...
//	&& !IS_SET(ch->act2, ACT2_PLR_EVENT) && !IS_SET(victim->act2, ACT2_PLR_EVENT)
		...
		if(ch!=victim && !IS_NPC(ch) && !IS_NPC(victim))
		{
         ...
         //Arena- players PK independent 
         if(   ch->pknorecall==0 && !IS_SET(ch->act2, ACT2_PLR_ARENA) 
//	&& !IS_SET(ch->act2, ACT2_PLR_EVENT)
	){ 
            ch->pknorecall=5; 
         } 
         //Arena- players PK independent 
...
if(IS_HERO(victim) && !IS_SET(victim->act2, ACT2_PLR_ARENA) 
//	&& !IS_SET(victim->act2, ACT2_PLR_EVENT)
	){ 
               if(GAMESETTING5(GAMESET5_HEROS_DONT_LOSE_XP_FOR_DYING)){ 
                  victim->printlnf("`cYou would have lost %d xp if you weren't a hero.`x", xp_amount);                          
                  }else{ 
                  victim->printlnf("`cYou have lost %d xp.`x", xp_amount); 
                  gain_exp( victim, -xp_amount);                        
               } 
            //Arena- players don't lose xp 
            }else if(IS_SET(victim->act2, ACT2_PLR_ARENA) 
//	&& !IS_SET(victim->act2, ACT2_PLR_EVENT)
	){ 
               victim->printlnf("`cYou have lost in the arena but not xp points.`x"); 
            ...
            //Arena- players perm damage independent 
...
//	&& !IS_SET(victim->act2, ACT2_PLR_EVENT)
                  ...
                  victim->println("`cLuckily you the gods have spared you serious injury.`x"); 
               } 
            } 
         } 
      } 
   }	
	sprintf( log_buf, "%s [lvl %d] was slain by %s at %s [lvl %d] [room %d] (%s)",
   ...
   //ARENA - raw_kill calls make_corpse 
   //avoid raw_kill, send him to a room fully dressed but defeated 
   if(!IS_SET(victim->act2, ACT2_PLR_ARENA) 
//	&& !IS_SET(ch->act2, ACT2_PLR_EVENT)
	){ 
   raw_kill( victim, ch ); 
  } 
act( "$n APPEARS FROM THE LAND OF THE LIVING.", victim, NULL, NULL, TO_ROOM );
 
   //ARENA VICTIM (LOSER) 
   //Send the defeated to the 'locker rooms' 
      if(IS_SET(victim->act2, ACT2_PLR_ARENA) 
//	|| IS_SET(ch->act2, ACT2_PLR_EVENT)
	){ 
         //char buf[MIL]; 
         //borrow the essential help from raw_kill 
         //without it PC'll go to position dead 
         //and 'see stars' and require assistance. 
         ...
         //reset PC to arena noncombatant 
         //frees arena for others to play 
         REMOVE_BIT(victim->act2, ACT2_PLR_ARENA); 
         REMOVE_BIT(victim->act2, ACT2_PLR_ARENA); 
         //If you use mobmemory, players will auto 
         //initiate a pkill upon leaving the arena 
         //so must remove it!!! Prob not the victim. 
         if(victim->mobmemory){ 
         victim->mobmemory = NULL; 
         } 
	...
	//special room for arena defeated. 
    ROOM_INDEX_DATA *room; 
    room = get_room_index(701); 
    char_from_room(victim); 
    char_to_room(victim, room); 

         //Move the winner out of the arena 
         if(SET_BIT( ch->act2, ACT2_PLR_ARENA )){ 
            ...
            //remove bit so he'll clear recall filters 
		 ...
		 do_recall( ch, "" ); 
            ch->printlnf( "`R[ARENA]:`Y GRATZ Victorious %s!.`x\n\r", ch->name ); 
            //reward? 
            int goldreward = number_range(20, 40); 
            ch->gold += goldreward; 
            ch->printlnf( "`R[ARENA]: `WYou win %d `Ygold!\n\r", goldreward); 
            //more? my dot gives qp reward 
	...
	if ( !IS_NPC(ch) && IS_NPC(victim) )
	{
		...
		{
			act( "`c$p holds:`x", ch, corpse, NULL, TO_CHAR );
...
void check_assist(char_data *ch,char_data *victim)
{
	...
	for (rch = ch->in_room->people; rch != NULL; rch = rch_next)
	{
		...
		{			
			...
			{
				do_emote(rch,"`cscreams and attacks!`x");
...
//	&& !IS_SET(ch->act2, ACT2_PLR_EVENT)
   ...
   { 
      ...
      if(!IS_SET(ch->act2, ACT2_PLR_ARENA) || !IS_SET(victim->act2, ACT2_PLR_ARENA)
//	|| !IS_SET(victim->act2, ACT2_PLR_EVENT) || !IS_SET(victim->act2, ACT2_PLR_EVENT)
	){ 
      ch->pknorecall=15; 
      ...
      if(GAMESETTING5(GAMESET5_DEDICATED_PKILL_STYLE_MUD)){ 
         ch->pknoquit   =2; 
         victim->pknoquit=2; 
      }else if(!IS_SET(ch->act2, ACT2_PLR_ARENA) && !IS_SET(ch->act2, ACT2_PLR_ARENA)){ 
         ch->pknoquit   =20; 
         victim->pknoquit=20; 
      }
/* else if(!IS_SET(ch->act2, ACT2_PLR_EVENT) && !IS_SET(ch->act2, ACT2_PLR_EVENT)){ 
         ch->pknoquit   =20; 
         victim->pknoquit=20; 
      } 
*/
	...
	*/
	...
	if (result && wield != NULL)
	{
		...
		if (ch->fighting == victim && IS_WEAPON_STAT(wield,WEAPON_POISON))
		{
			...
			{
				victim->println("`cYou feel poison coursing through your veins.`x");
				act("`c$n is poisoned by the venom on $p.`x",
		...
		if (ch->fighting == victim && IS_WEAPON_STAT(wield,WEAPON_VAMPIRIC))
		{
			...
			ch->hit += dr;
			act("`c$p draws life from $N.`x",ch,wield,victim,TO_NOTVICT);
			act_with_autodam_to_char("`c$p draws life from $N.`x",ch,wield,victim,dr);
			dr=temp_hps-victim->hit;
			act_with_autodam_to_char("`cYou feel $p drawing your life away.`x",victim,wield,NULL,dr);
		}
		...
		if (ch->fighting == victim && IS_WEAPON_STAT(wield,WEAPON_FLAMING))
		{
			...
			act("`c$N is burned by $p.`x",ch,wield,victim,TO_NOTVICT);
			act_with_autodam_to_char("`c$N is burned by $p.`x",ch,wield,victim,dr);
			act_with_autodam_to_char("`c$p sears your flesh.`x",victim,wield,NULL,dr);			
		...
		if (ch->fighting == victim && IS_WEAPON_STAT(wield,WEAPON_FROST))
		{
			...
			act("`c$p freezes $N.`x",ch,wield,victim,TO_NOTVICT);
			act_with_autodam_to_char("`c$p freezes $N.`x",ch,wield,victim,dr);
			act_with_autodam_to_char("`cThe cold touch of $p surrounds you with ice.`x",
		...
		if (ch->fighting == victim && IS_WEAPON_STAT(wield,WEAPON_SUCKLE))
        {
            ...
            act("`c$p draws mystical energy from $n.`x",victim,wield,NULL,TO_ROOM);
            act("`cYou feel $p drawing your mystical energy away.`x",
        ...
        {
            dam = number_range(1, wield->level / 5 + 1);
            act("`c$p saps the strength from $n.`x",victim,wield,NULL,TO_ROOM);
            act("`cYou feel $p sapping your strength away.`x",
                victim,wield,NULL,TO_CHAR);
            victim->move-= dam;
            ch->move += dam/2;
        }
        ...
        if (ch->fighting == victim && IS_WEAPON_STAT(wield,WEAPON_ANNEALED))
        {
            ...
            if (!saves_spell(wield->level/4 + dam/20,victim,DAM_BASH)){
				victim->printlnf("`cYou see stars as your head `Crings`x!%s",
					autodamtext(victim, dr));
				ch->printlnf("`cYour opponent looks `Bstunned`x!%s",
					autodamtext(ch, dr));
		...
		{			
			...
			act("`c$N is struck by lightning from $p.`x",ch,wield,victim,TO_NOTVICT);
			act_with_autodam_to_char("`c$N is struck by lightning from $p.`x",ch,wield,victim,dr);
			act_with_autodam_to_char("`cYou are shocked by $p`x",victim,wield,NULL,dr);			
	    ...
	    bool show) 
{
		...
		|| IS_SET( ch->affected_by, AFF_SNEAK))
	{
		...
		act( "`c$n is revealed from the shadows.`x", ch, NULL, NULL, TO_ROOM );
	}
	...
	*/
	...
	if(victim->mounted_on){
		...
		if(percent_of_max_hits>=10){
			if(!IS_NPC(victim)) // player
			{
				...
				{
					act("`cYou are knocked from your mount!`x\r\n", victim, NULL, NULL, TO_CHAR);
					act("`c$n is knocked from $s mount!`x\r\n", victim, NULL, NULL, TO_ROOM);
					dismount(victim);
    ...
    { 
      act( "`c$n appears to be subdued.`x", victim, NULL, NULL, TO_ROOM ); 
      victim->println("`cYou have been subdued.`x"); 

      if(!IS_NPC(victim) && !IS_SET(victim->act2, ACT2_PLR_ARENA) 
//	&& !IS_SET(victim->act2, ACT2_PLR_EVENT)
	){ 
    ...
    else
    {
		...
		case POS_MORTAL:
			act( "`c$n is mortally wounded, and will die soon, if not aided.`x",
				victim, NULL, NULL, TO_ROOM );
			victim->println("`cYou are mortally wounded, and will die soon, if not aided.`x");
			break;
			
		case POS_INCAP:
			act( "`c$n is incapacitated and will slowly die, if not aided.`x",
				victim, NULL, NULL, TO_ROOM );
			victim->println("`cYou are incapacitated and will slowly die, if not aided.`x");
			break;
			
		case POS_STUNNED:
			act( "`c$n is stunned, but will probably recover.`x",
				victim, NULL, NULL, TO_ROOM );
			victim->println("You are stunned, but will probably recover.`x");
			break;
			
		case POS_DEAD:
			act( "`c$n is DEAD!!`x", victim, 0, 0, TO_ROOM );
			victim->println("You have been killed.`x");

			/*Military Points gains & losses*/
			
			if(!IS_NPC(ch) && !IS_NPC(victim))
			{
				int tempmps = 0;
			
				if(victim->pcdata->military_points >= 10)
				{
				tempmps = victim->pcdata->military_points/4; /*used to store the military points lost when a player loses*/
				victim->pcdata->military_points = victim->pcdata->military_points - tempmps;
				ch->pcdata->military_points = ch->pcdata->military_points + tempmps;
				victim->printlnf("`#`RYou have been defeated in combat!\n\r This has caused you to lose %d Military Points`^.", tempmps);
			ch->printlnf("`#`RYou have defeated %s in combat!\n\r This has caused you to gain %d Military Points`^.", victim->name, tempmps);
				}
				else
				{
				tempmps = victim->pcdata->military_points;
				...
				if(victim->bounty > 0)
				{
				ch->gold += victim->bounty;
                       		ch->printlnf("`cYou have collected a bounty of %d gold from %s.`x", victim->bounty, victim->name);
                       		info_broadcast(ch,"%s has slain %s collecting a bounty of %d.", ch->name, victim->name, victim->bounty);
                      		victim->bounty = 0;
				}
			}
		break;
		...
		default:
			...
			{
				victim->println("`cThat caused you a lot of pain.`x"); 
			}
			if ( victim->hit < victim->max_hit / 4 )
			{
				victim->println("`RYou are bleeding a lot.`x"); 
			}
			break;
		}
...
bool is_safe(char_data *ch, char_data *victim)
{
	...
	if( IS_NPC(victim) && IS_SET(victim->act2,ACT2_AVOIDS_ALL_ATTACKS) ){
		ch->printlnf("`c%s cowers away from your attack.`x", PERS(victim, ch));
		return true;
	}
	...
	{
		ch->println("`cNot in this room.`x");
		return true;
	}
	...
	{
		ch->println("`cNot in an `BOOC `croom.`x");
		return true;
	}
    ...
    if (IS_NPC(victim))
    {
		if( ch->level<6 || (!IS_LETGAINED(ch) && GAMESETTING_LETGAINING_IS_REQUIRED) ){
			...
			{
				ch->wrapln("`cThat is a players pet, which could start a pkill fight,`x "
					"`csince you are either below level 6 or not letgained you can't`x "
					"`cstart pkill combat.`x");
				return true;
			}
		}
		...
		if (HAS_CONFIG(ch, CONFIG_DISALLOWED_PKILL))
		{
			...
			{
				ch->wrapln("`cThat is a players pet, which could start a pkill fight,`x "
					"`csince you are pkill restricted you can't start pkill combat.`x");
				return true;
			}
		}
		...
		*{
		*     ch->println("`cThe shopkeeper wouldn't like that.`x");
		*     return true;
		*}
		*/
		...
		*  {
		*     ch->println("`cThe gods would not approve.`x");
		*     return true;
		*  }
		*/
		...
		if (!IS_NPC(ch))
		{
			...
			/*    if (IS_SET(victim->act,ACT_PET))
			*   {
			*  act("`cThere is no point to killing someone's pet...`x",
			...
			*/
			...
			{
			ch->println("`cYou don't own that animal.`x");
			return true;
			}
			*/
		}
    ...
    else  // killing players
    {
		...
		if(IS_SET( ch->dyn, DYN_IS_BEING_ORDERED ) && ch->master)
		{
			if((ch->master->level<6) || (!IS_LETGAINED(ch->master) && GAMESETTING_LETGAINING_IS_REQUIRED) ){
				// tell charmie
				ch->println("`cYour master isn't letgained or above level 6.`x");
				// tell master
				ch->master->println("`cYou can not order a pet to pkill, steal from players etc,`x");
				ch->master->println("`ctill level 6 and you have been letgained.`x");
				ch->master->println("`c(See HELP DEATH and HELP LETGAIN)`x");
				return true;
			}
		}
		...
		if (IS_NPC(ch))
		{
			...
			{
				ch->println("`cPeople are your friends!`x");
				return true;
			}
		...
		{
			if ((ch->level<6) || (!IS_LETGAINED(ch) && GAMESETTING_LETGAINING_IS_REQUIRED) ){
				ch->println("`cYou can not player kill, steal from players etc,`x");
				ch->println("`ctill level 6 and you have been letgained.`x");
				ch->println("`c(See HELP DEATH and HELP LETGAIN)`x");
				return true;
			}
			...
			{
				ch->println("`cYou can not player kill, steal from players etc,`x");
				ch->println("`cdue to a pkill restriction sorry.`x");
				return true;
			}
			...
			if(HAS_CONFIG2(victim,CONFIG2_NOPKILL)){
				ch->println("`cA mysterious force prevents you from doing that.`x");
				return true;
			}

			if(((victim->level<11))&&(ch->pkool>0))
			{
				ch->println("`cYou cannot kill the weak so soon after doing so.`x");
				return true;
			}
			if(victim->pksafe>0)
         { 
            //More arena allowance 
            if(IS_SET(victim->act2, ACT2_PLR_ARENA)){ 
               victim->pksafe = 0; 
               return false; 
            }else{ 
            ch->println("`cHe has died too recently, leave him alone.`x"); 
               return true; 
            } 
         } 
			
//added to stop non-clanned from killing or being killed
if(!victim->clan && !IS_SET(victim->in_room->room_flags, ROOM_ARENA)) 
{ 
     ch->printlnf("`cYou cannot pkill someone that is not enlisted.`x"); 
     return true; 
} 

if (!ch->clan && !IS_SET(ch->in_room->room_flags, ROOM_ARENA)) 
{ 
     ch->printlnf("`cYou are not enlisted, therefore, you cannot pkill.`x"); 
     return true; 
}
...
bool check_parry( char_data *ch, char_data *victim )
{
	...
	act( "`c$n parries your attack.`x", victim, NULL, ch, TO_VICT );
...
bool check_shield_block( char_data *ch, char_data *victim )
{
	...
	act( "`cYou block $n's attack with your shield.`x", 
		ch, NULL, victim, TO_VICT    );
	act( "`c$n blocks your attack with a shield.`x", 
...
bool check_dodge( char_data *ch, char_data *victim )
{
	...
	act( "`c$n dodges your attack.`x", victim, NULL, ch, TO_VICT);
...
void set_fighting( char_data *ch, char_data *victim )
{
	...
	if(!IS_NPC(ch))
	{
		...
		{
			victim->println("`cYou dont return the attack, your wizi above them.`x");
			return;
		}
		...
		if (IS_NPC(victim) 	&& !IS_SET(ch->dyn,DYN_DOING_DAMAGE)
			...
			&& !HAS_HOLYLIGHT(ch))
		{
			victim->println("`cYou dont return the attack, your UNSEEN to them.`x");
			return;
		}
	}
	...
	&&   get_eq_char( ch, WEAR_SHEATHED ) != NULL )
	{
		...
		act( "`cYou hastily draw $p.`x", ch, obj, NULL, TO_CHAR );
		act( "`c$n hastily draws $p.`x", ch, obj, NULL, TO_ROOM );
	...
	&&   get_eq_char( victim, WEAR_SHEATHED ) != NULL )
	{
		...
		act( "`cYou hastily draw $p.`x", victim, obj, NULL, TO_CHAR );
		act( "`c$n hastily draws $p.`x", victim, obj, NULL, TO_ROOM );
    ...
    if (TRUE_CH(ch)->pcdata && IS_SET(TRUE_CH(ch)->act,PLR_AUTOSUBDUE)){
		...
		ch->println("`cYou are subduing in this fight.`x");
	}else{
		REMOVE_BIT(ch->dyn,DYN_CURRENT_SUBDUE);
		ch->println("`cYou are fighting to kill in this fight.`x");
    }
...
void instant_decay_corpse(char_data *ch)
{
    ...
    act( "`c$n disappears into thin air right before your eyes!`x", ch, NULL, NULL, TO_ROOM );
	...
	if ( ch->gold > 0 )
	{
	    ...
	    act( "`cA pile of coins falls to the ground from the space\r\nonce occupied by $n.`x", ch, NULL, NULL, TO_ROOM );
	    ch->gold = 0;
	    ch->silver = 0;
	}
    }
	...
	else if (floating)
	{
		...
		if (obj->contains != NULL)
		{
		    ...
		    act("`c$p evaporates,scattering its contents.`x",
		    ch,obj,NULL,TO_ROOM);
		    ...
		    act("`c$p evaporates.`x", ch,obj,NULL,TO_ROOM);

		 extract_obj(obj);
	    }
	    else
	    {
		act("`c$p falls to the floor.`x",ch,obj,NULL,TO_ROOM);
		obj_to_room(obj,ch->in_room);
	    }
	}
	else
	{           
	    act("`c$p falls to the floor from the space\r\nonce occupied by $n.`x",ch,obj,NULL,TO_ROOM);
	    obj_to_room( obj, ch->in_room );
	}
    }
		...
		if( IS_SET( obj->extra_flags, OBJEXTRA_INVENTORY )){
			...
			if (floating)
			{
				...
				{ 
					...
					{
						OBJ_DATA *in, *in_next;
						act("`c$p evaporates,scattering its contents.`x", ch,obj,NULL,TO_ROOM);
						for (in = obj->contains; in != NULL; in = in_next)
						{
							in_next = in->next_content;
							obj_from_obj(in);
...
void death_cry( char_data *ch, char_data *killer )
{
    ...
    {
    case  0: msg  = "`c$n hits the ground ... DEAD.`x"; 
		break;
    case  1: 
		if (ch->material == 0)
		{
	...
	{
		act( "`cYou managed to kill $N before they could let out a final cry.`x", killer, NULL, ch, TO_CHAR );
...
void raw_kill( char_data *victim, char_data *killer )
{
    ...
    if (victim->ridden_by)
    {
		victim->ridden_by->println("`cYour mount has been killed, you fall to the ground!`x");
	...
	*/
	...
	for ( gch = ch->in_room->people; gch != NULL; gch = gch->next_in_room )
	{
		...
		/*      Taken out, add it back if you want it
			...
			{
			gch->println("`cYour level is too high for this group.`x");
			continue;
			}
			
			  if ( gch->level - lch->level <= -5 )
			  {
			  gch->println("`cYour level is too low for this group.`x");
			  continue;
			  }
		*/
		...
		if(GAMESETTING5(GAMESET5_ADDICT_XP_BONUS)){
			...
			{
				gch->println("`cYou gain an addict bonus.`x");
		...
		if(IS_SET(gch->comm, COMM_BUILDING)){
			...
			gch->printlnf("`cYou receive %d experience points.`x", xp );
			int bonus;
			bonus = xp;
			if (double_exp)
			{
    			xp += bonus;
    			ch->printlnf("`cExpierence gains are doubled, you get %d bonus expienece points.`x", bonus );
			}
			gain_exp(gch, xp);
		}
...
void dam_message( char_data *ch, char_data *victim,int dam,int dt,bool immune )
{
    ...
    if(GAMESETTING3(GAMESET3_STORM_DAMAGE_MESSAGES)){
		...
		if ( dam > victim->max_hit ) { vs = "`cslay"; vp = "`cslays";			}
		else if ( dam == 0  ) { vs = "`cmiss";		vp = "`cmisses";			}
		else if ( dam <= 2  ) { vs = "`chardly touch";	vp = "`chardly touches";		}
		else if ( dam <= 10  ) { vs = "`cgraze";		vp = "`cgrazes";			}
		else if ( dam <= 20  ) { vs = "`cbarely hit";	vp = "`cbarely hits";			}
		else if ( dam <= 30  ) { vs = "`chit";			vp = "`chits"; 		}
		else if ( dam <= 40 ) { vs = "`cwound lightly";	vp = "`cwounds lightly";			}
		else if ( dam <= 50 ) { vs = "`chit hard";		vp = "`chits very hard";	}
		else if ( dam <= 60 ) { vs = "`cwound harshly";	vp = "`cwounds harshly";		}
		else				   { vs = "`chit with perfection";
					 vp = "`chits with perfection"; 					}
	}

	char vsbuf[MSL], vpbuf[MSL];
	sprintf(vsbuf, "`c%s", vs);
	sprintf(vpbuf, "`c%s", vp);
	vs=vsbuf;
	vp=vpbuf;
    ...
    if ( dt == TYPE_HIT )
    {
		if (ch  == victim)
		{
			sprintf( buf1, "`c$n %s $melf%c",vp,punct);
			sprintf( buf2, "`cYou %s yourself%c",vs,punct);
		}
		else
		{
			 sprintf( buf1, "`c$n %s $N%c",  vp, punct );
			 sprintf( buf2, "`cYou %s $N%c", vs, punct );
			 sprintf( buf3, "`c$n %s you%c", vp, punct );
		}
	}
	else
	{
		...
		if (immune)
		{
			if (ch == victim)
			{
				sprintf(buf1,"`c$n is unaffected by $s own %s.`x`",attack);
				sprintf(buf2,"`cLuckily, you are immune to that.`x");
			} 
			else
			{
				sprintf(buf1,"`c$N is unaffected by $n's %s!`x",attack);
				sprintf(buf2,"`c$N is unaffected by your %s!`x",attack);
		...
		else
		{
			if (ch == victim)
			{
				sprintf( buf1, "`c$n's %s %s $m%c`x",attack,vp,punct);
				sprintf( buf2, "`cYour %s %s you%c`x",attack,vp,punct);
			}
			else
			{
				sprintf( buf1, "`c$n's %s %s $N%c`x",  attack, vp, punct );
				sprintf( buf2, "`cYour %s %s $N%c`x",  attack, vp, punct );
				sprintf( buf3, "`c$n's %s %s you%c`x", attack, vp, punct );
			}
		}
	}
    ...
    else
    {
		...
		{
			strcat(buf3, FORMATF("`c [%d]`x", dam));
		}

		sprintf(bf,"`c%s`x",buf3);
...
void entangle( char_data *ch, char_data *victim )
{
	...
	{
		act("`c$S weapon won't budge!`x",ch,NULL,victim,TO_CHAR);
		act("`c$n tries to entangle your weapon but your weapon won't budge!`x",ch,NULL,victim,TO_VICT);
		act("`c$n tries to entangle $N's weapon, but fails.`x",ch,NULL,victim,TO_NOTVICT);
		return;
	}
	
    act( "`c$n entangles $p and grabs it!!`x",	ch, obj, victim, TO_VICT    );
    act( "`cYou entangle $N's weapon!`x",  ch, NULL, victim, TO_CHAR    );
    act( "`c$n entangles $N's weapon!`x",  ch, NULL, victim, TO_NOTVICT );
...
void do_bash( char_data *ch, char *argument )
{
		...
		&&       ch->level < skill_table[gsn_bash].skill_level[ch->clss]))
	{
		if (!IS_CONTROLLED(ch))
		{
			ch->println("`cBashing? What's that?`x");
			return;
		}
	}
	...
	if (arg[0] == '\0')
	{
		...
		if (victim == NULL)
		{
			ch->println("`cBut you aren't fighting anyone!`x");
			return;
		}
	}
	...
	if (victim == ch)
	{
		ch->println("`cYou try to bash your brains out, but fail.`x");
		return;
	}

	if (IS_AFFECTED(victim, AFF_FLYING) && !IS_AFFECTED(ch, AFF_FLYING))
	{
		ch->println("`cYou can't reach them since they are flying and you are not.`x");
		return;
	}
    ...
    if (!IS_NPC(victim) 
		...
		{   /*
			act("`c$n tries to bash you, but you dodge it.`x",ch,NULL,victim,TO_VICT);
			act("`c$N dodges your bash, you fall flat on your face.`x",ch,NULL,victim,TO_CHAR);
    ...
    {	
		...
		if(dr){
			act("`c$n sends you sprawling with a powerful bash!$t`x", 
				ch, autodamtext(victim, dr) ,victim,TO_VICT);
			act("`cYou powerfully slam into $N, and send $M flying!$t`x",
				ch, autodamtext(ch, dr) ,victim,TO_CHAR);
			act("`c$n sends $N sprawling with a powerful bash.`x",
				ch,NULL,victim,TO_NOTVICT);
		}else{
			act("`c$n sends you sprawling with a bash!$t`x",
				ch, autodamtext(victim, dr),victim,TO_VICT);
			act("`cYou slam into $N, and send $M flying!$t`x",
				ch, autodamtext(ch, dr) ,victim,TO_CHAR);
			act("`c$n sends $N sprawling with a bash.`x",
				ch,NULL,victim,TO_NOTVICT);
		}
	...
	else
	{
		...
		act("`cYou fall flat on your face!`x", ch,NULL,victim,TO_CHAR);
		act("`c$n falls flat on $s face.`x", ch,NULL,victim,TO_NOTVICT);
...
void do_dirt( char_data *ch, char *argument )
{
    ...
    {
		ch->println("`cYou get your feet dirty.`x");
		return;
    }
    ...
    if (arg[0] == '\0')
    {
		...
		if (victim == NULL)
		{
		    ch->println("`cBut you aren't in combat!`x");
		    return;
		}
    }
	else if ((victim = get_char_room(ch,arg)) == NULL)
	...
	{
		ch->println("`cYou can't dirt kick while you fly.`x");
		return;
	}

	if (victim == ch)
    {
		ch->println("`cVery funny.`x");
		return;
    }
	...
	switch(ch->in_room->sector_type)
	{
		...
		case(SECT_HOME):              chance -= 20;   break;
		default:						chance	=  0;	break;
    }

	if ( chance == 0 )
    {
		ch->println("`cThere isn't any dirt to kick.`x");
		return;
    }
	...
	{
		act("`c$n is totally missed by the dirt!`x",victim,NULL,NULL,TO_ROOM);       
		act("`c$n fails to kick dirt properly!`x",ch,NULL,victim,TO_VICT);
...
void do_trip( char_data *ch, char *argument )
{
    ...
    {
		ch->println("`cTripping?  What's that?`x");
		return;
    }
...
// standard no mount checks etc
	if(IS_RIDING(ch)) 
	{
		ch->println("`cYou can't trip others while riding a creature.`x");
		return;
	}
    ...
    if (victim == ch)
    {
		ch->println("`cYou fall flat on your face!`x");
...
void do_kill( char_data *ch, char *argument )
{
   ...
   if ( !IS_NPC(victim)  // directly attacking the player 
          && !IS_SET(ch->act2, ACT2_PLR_ARENA) // Allow arena players to fight 
   && !IS_SET(victim->act2, ACT2_PLR_ARENA)
//          && !IS_SET(ch->act2, ACT2_PLR_EVENT)  
//   && !IS_SET(victim->act2, ACT2_PLR_EVENT)
	) 
    ...
    if ( victim == ch )
    {
		ch->println("`cYou hit yourself.  Ouch!`x");
    ...
    if ( ch->position == POS_FIGHTING )
    {
		ch->println("`cYou do the best you can!`x");
		return;
    }
...
void do_pkill( char_data *ch, char *argument )
{
	...
	if(!IS_NPC(ch) && GAMESETTING(GAMESET_PEACEFUL_MUD)){
		ch->println("`cThis is a peaceful mud... pkilling is not permitted.`x");
		return;
	}
	...
	sprintf(oarg, "`c%s`x", argument);

    one_argument( argument, arg );

    if ( arg[0] == '\0' )
    {
		ch->println("`cPkill whom?`x");
		return;
    }
    ...
    if ( ch->position == POS_FIGHTING )
    {
		ch->println("`cYou are already doing your best!`x");
		return;
    }
...
void do_pbackstab( char_data *ch, char *argument )
{
	...
	if(IS_MOUNTED(ch)){
		ch->println("`cYou can't backstab cause you are being ridden.`x");
		return;
	}

	if(IS_RIDING(ch)){
		ch->println("`cYou can't backstab while riding a creature.`x");
		return;
	}
    ...
    if (arg[0] == '\0')
    {
		ch->println("`cPbackstab whom?`x");
		return;
    }
	
    if (ch->fighting != NULL)
    {
		ch->println("`cYou're facing the wrong end.`x");
		return;
    }
    ...
    if ( victim == ch )
    {
		ch->println("`cHow can you sneak up on yourself?`x");
		return;
    }
	...
	{
		ch->println("`cYou can't backstab someone on a mount unless you are flying.`x");
		return;
	}
    ...
    {
		ch->println("`cYou need to wield a dagger to backstab.`x");
		return;
    }
	...
	if (osec)
	{
		...
		{
			ch->println("`cYour secondary weapon must be a dagger as well to backstab.`x");
			return;	
		}		
	}
	...
	if ( IS_AFFECTED(victim, AFF_FLYING) 
		...
		&& number_range(1,3)==1)
    {
		ch->println("`cYou attempt to backstab them, but they fly out of the way and you miss.`x");
...
void do_circle( char_data *ch, char *argument )
{
	...
	{
		ch->println("`cWhat would you know about circling people?`x");
		return;
	}

    obj=get_eq_char( ch, WEAR_WIELD);
    if(!obj || obj->value[0]!=WEAPON_DAGGER){
		ch->println("`cYou must wield a dagger to circle.`x");
        return;
    }

	victim = ch->fighting;
    if(!victim){
		ch->println("`cYou aren't fighting anyone.`x");
		return;
    }

	if (IS_AFFECTED(ch,AFF_BLIND)){
		ch->println("`cYou are blinded and can't find a path to circle them.`x");
	...
	if(!GAMESETTING3(GAMESET3_CIRCLE_ALLOWED_WITHOUT_CLEAR_SHOT) && !IS_NPC(ch)){
		if ( victim->fighting == ch ){
			act( "`c$N turns with you, not offering you a clear shot.`x", ch, NULL, victim, TO_CHAR );
			return;
		}
	}
...
void do_backstab( char_data *ch, char *argument )
{
	...
	sprintf (buf,"`c%s`x", argument);
    one_argument( argument, arg );

    if (arg[0] == '\0')
    {
		ch->println("`cBackstab whom?`x");
		return;
    }
	...
	if ( victim->fighting == ch )
    {
		ch->println("`cThey already appear to be fighting you!`x");
		return;
    }
    ...
    {
		act( "`c$N is too suspicious of your attack... you can't sneak up.`x",
			ch, NULL, victim, TO_CHAR );
		return;
    }
...
void do_flee( char_data *ch, char *)
{
	...
	if( IS_AFFECTED(ch,AFF_SLOW) 
		...
		&& ch->mounted_on!=NULL)
	{
		ch->println("`cYou feel too lethargic to run.`x");
		return;
	}
	...
	{
		ch->println("`cYour RAGE within is so strong you can't draw yourself away from the fight!`x");
		return;
	}
	...
	||   IS_SET( victim->dyn, DYN_IS_CUTTING_OFF ))
	{
		...
		if ( polearm )
		{
			if ( polearm->value[0] == WEAPON_POLEARM )
			{
				...
				if ( successful )
				{
					act( "`c$N blocks your escape with $p.`x", ch, polearm, victim, TO_CHAR );
					act( "`c$n tries to flee, but you cut $m off.`x", ch, NULL, victim, TO_VICT );
					act( "`c$n tries to flee, but $N makes escape impossible.`x", ch, NULL, victim, TO_NOTVICT );
    ...
    for ( attempt = 0; attempt < 10; attempt++ )
    {
		...
		if ( !IS_NPC(ch) )
		{
//annoying as hell			msp_to_room(MSPT_ACTION, MSP_SOUND_FLEE, 0,	ch, true, false);	

			ch->println("You flee from combat!");
			if( HAS_CLASSFLAG(ch, CLASSFLAG_SAFE_FLEE_FROM_COMBAT)
				&& (number_percent() < 3*(ch->level/2) ) )
			{
				ch->println("`cYou snuck away safely.`x");
			}else{
				ch->printlnf("`cYou lost %d exp.`x", game_settings->xp_loss_for_fleeing); 
    ...
    ch->println("`cYou couldn't escape!`x");
    return;
}
...
void do_retreat( char_data *ch, char *argument )
{
	...
	{
		ch->println("`cYou do not know how to retreat, try fleeing.`x");
		return;
    }
	...
	{
		ch->println("`cYour RAGE within is so strong you dont want to retreat!`x");
		return;
	}
    ...
    if ( IS_NULLSTR( arg )) {
		ch->println("`cYou must provide a direction in which to retreat.`x");
		return;
    }
    ...
    if ( door == -1 )
	{
		ch->println("`cBad direction. Which way?`x"); 
		return;
    }
    ...
    if (( pexit = was_in->exit[door] ) == 0
		...
		|| ( IS_NPC( ch )
		&&   IS_SET( pexit->u1.to_room->room_flags, ROOM_NO_MOB ))) {
		ch->println("`cYou can't retreat in that direction.`x");
		return;
    }
    ...
    if ( number_percent() < chance )
    {
        ...
        if (( now_in = ch->in_room ) == was_in ) {
			ch->println("`cThat got you nowhere.`x");
			return;
		}  //  Loopy maze room check
		
		ch->in_room = was_in;	
        act("`c$n has retreated.`x", ch, NULL, NULL, TO_ROOM );
        ch->in_room = now_in;
		
		if ( !IS_NPC( ch )) {
			msp_to_room(MSPT_ACTION, MSP_SOUND_FLEE, 0,	ch, true, false);	
			ch->println("`cYou have succesfully retreated from the fight.`x");
			if (ch->move)
			{
				ch->move= (ch->move*8/10)+1; //lose just under 20% of your moves
			}

			ch->println("`cYou lost 5 exp.`x");
			gain_exp( ch, -5 );
			if ( IS_HERO( ch ))
				do_heroxp( ch, -10 );
		}
		...
		ch->println("`cYou couldn't get away.`x");
...
void do_rescue( char_data *ch, char *argument )
{
    ...
    if ( victim == ch )
    {
		ch->println("`cWhat about fleeing instead?`x");
		return;
    }
    ...
    if ( !IS_NPC(ch) && IS_NPC(victim) )
    {
		ch->println("`cDoesn't need your help!`x");
		return;
    }

    if ( ch->fighting == victim )
    {
		ch->println("`cToo late.`x");
		return;
    }

    if ( ( fch = victim->fighting ) == NULL )
    {
		ch->println("`cThat person is not fighting right now.`x");
		return;
    }
    ...
    {
		ch->println("`cYou fail the rescue.`x");
...
void do_kick( char_data *ch, char *argument)
{
    ...
    {
		ch->println("`cYou better leave the kicking for someone who knows how.`x");
		return;
    }
	...
	{
		ch->printlnf("`c'%s' is not here.`x", argument);
		return;
	}
	
	if (victim == ch)
	{
		ch->println("`cYou try to kick yourself but fail.`x");
		return;
	}
...
void do_disarm( char_data *ch, char *argument )
{
    ...
    {
		ch->println("`cYou don't know how to disarm opponents.`x");
		return;
    }
    ...
    {
		ch->println("`cYou must wield a weapon to disarm.`x");
		return;
    }
	...
	if (( obj = get_eq_char( victim, WEAR_SECONDARY) ) == NULL )
    {
		ch->println("`cYour opponent is not wielding a weapon.`x");
		return;
    }

	// No disarm when the victim has vicegrip
    if (IS_AFFECTED2(victim, AFF2_VICEGRIP)){
		act("`cYour opponent has a solid grip on their weapon! You have no chance!`x",ch,NULL,victim,TO_CHAR);
...
void do_surrender( char_data *ch, char * )
{
    ...
    if (!mob)
	{
		ch->println("`cBut you're not fighting!`x");
		return;
    }

	if (!IS_NPC(mob)){
		ch->println("`cYou can't surrender in pkill fights!`x");
		return;
	}
    ...
    act( "`cYou surrender to $N!`x", ch, NULL, mob, TO_CHAR );
    act( "`c$n surrenders to you!`x", ch, NULL, mob, TO_VICT );
    act( "`c$n tries to surrender to $N!`x", ch, NULL, mob, TO_NOTVICT );
    ...
    {
		act( "`c$N seems to ignore your cowardly act!`x", ch, NULL, mob, TO_CHAR );
...
{
    ch->println("`cIf you want to SLAY, spell it out.`x");
    return;
}
...
void do_tail( char_data *ch, char *argument )
{
    ...
    {
		ch->println("`cDo you even have a tail?`x");
		return;
    }
    ...
    if (victim == ch)
    {
	    ch->println("`cDo you really want to hit yourself?`x");
		return;
    }

	if ( IS_AFFECTED( victim, AFF_FLYING ))
	{
		ch->println("`cTheir feet are not on the ground.`x");
		return;
	}
    ...
    if ( IS_NPC(victim) &&
	    ...
	    {
			ch->println("`cKill stealing is not permitted.`x");
			return;
	    }
	...
	&& chance < get_skill( victim, gsn_dodge ))
    {   
		act("`c$n tries to smash you with a tail.`x",ch,NULL,victim,TO_VICT);
		act("`c$N dodges your tail, you falter off balance.`x",ch,NULL,victim,TO_CHAR);
...
//		return;
    ...
    {
		act("`c$n sweeps your feet from under you with $s tail!`x",	ch,NULL,victim,TO_VICT);
	    act("`cYou slam a tail into $N, and send $M flying!`x",ch,NULL,victim,TO_CHAR);
	    act("`c$n sends $N sprawling with a powerful smash from a tail.`x",	ch,NULL,victim,TO_NOTVICT);
...
//    damage(ch,victim,0,gsn_tail,DAM_BASH,false);
		act("`cYou fall flat on your face!`x", ch,NULL,victim,TO_CHAR);
		act("`c$n falls flat on $s face.`x", ch,NULL,victim,TO_NOTVICT);
	    act("`cYou evade $n's tail, causing $m to falter and lose balance.`x", ch,NULL,victim,TO_VICT);
...
void do_claw( char_data *ch, char *)
{
    ...
    {
		ch->println("`cYou do not know how to claw.`x");
		return;
    }
...
void do_gouge( char_data *ch, char *)
{
    ...
    {
		ch->println("`cYou do not know how to gouge.`x");
		return;
    }
    ...
    */
    ...
    if ( get_skill(ch,gsn_gouge) > number_percent())
    {
		...
		if ( saves_spell( ch->level*3/2, victim, DAM_POISON)
			|| HAS_CLASSFLAG(victim, CLASSFLAG_POISON_IMMUNITY))
		{
			act("`c$n turns slightly green, but it passes.`x",victim,NULL,NULL,TO_ROOM);
			victim->println("`cYou feel momentarily ill, but it passes.`x");
			check_improve(ch,gsn_gouge,false,1);
		}else{
			AFFECT_DATA af;
			
			victim->println("`cYou feel poison coursing through your veins.`x");
			act("`c$n looks very ill.`x",victim,NULL,NULL,TO_ROOM);
...
void do_gore( char_data *ch, char *argument )
{
    ...
    if ( !obj && !IS_SET( ch->parts, PART_HORNS )) {
        ch->println("`cYou have no horns to gore with.`x");
        return;
    }
    
	if ( obj ) {
        if ( !IS_OBJ_STAT( obj, OBJEXTRA_HORNED )) {
            act ( "`cThere are no horns on $p!`x", ch, obj, NULL, TO_CHAR );
            return;
        }
    }
...
// standard no mount checks etc
	if(IS_MOUNTED(ch)) 
	{
		ch->println("`cYou can't gore cause you are being ridden.`x");
		return;
	}
	if(IS_RIDING(ch)) 
	{
		ch->println("`cYou can't gore others while riding a creature.`x");
		return;
	}
    ...
    if (( chance = get_skill( ch, gsn_gore )) == 0
		...
		ch->println("`cYou are not proficient enough in that skill.`x");
        return;
    }
    ...
    if ( IS_NULLSTR(arg) ) {
        ...
        ch->printlnf("`cYou can't seem to find '%s' here.`x", arg);
        return;
    }
...
// Inserted possible cheat deterents
    ...
    if ( get_skill( ch,gsn_gore ) > number_percent() ) {
        ch->println("`cYou set your head down and charge!`x");
        ...
        if ( number_percent() <= 50 ) {
            ch->println("`cYou charge so wildly that you knock yourself down too.`x");
            act( "`c$n knocks himself down after charging you.`x",
                  ch, NULL, victim, TO_VICT );
            act( "`cWith a wild flourish, $n goes down in a heap.`x",
                  ch, NULL, victim, TO_NOTVICT );
            ch->position = POS_RESTING;
        }
    }else{
        ...
        check_improve( ch, gsn_gore, false, 1 );
		ch->println("`cYou blindly charge past your mark, missing by a mile!`x");
        act("`c$n tries to gore $N but fails miserably.`x",
            ch, NULL, victim, TO_NOTVICT);
        act("`c$n runs right past you and trips a step later.`x",
            ch, NULL, victim, TO_VICT );
        check_improve( ch, gsn_gore, false, 1 );
        ch->position = POS_RESTING;        
    }
...
{
    if (!ch->fighting){
        ch->println("`cYou aren't currently in combat, use autosubdue.`x"); 
        return;
    }
	
    if (IS_SET(ch->dyn,DYN_CURRENT_SUBDUE))
    {
        ch->println("`cYou will now fight this fight to the death.`x");
        REMOVE_BIT(ch->dyn,DYN_CURRENT_SUBDUE);
    }
    else
    {
        ch->println("`cYou will attempt to subdue your current opponent.`x");
        SET_BIT(ch->dyn,DYN_CURRENT_SUBDUE);
    }
}
...
//			attacker->println("You can't be ordered to initiate PK combat.");
	...
	if(IS_SET(type,CIT_GENERAL)){
		...
		{
//			attacker->println("`cKill stealing is not permitted.`x");
			return false;
		}
	};
...
//Allow arena players to fight.    
   if(!IS_NPC(victim) && !IS_NPC(attacker) 
      && IS_SET(victim->act2, ACT2_PLR_ARENA) 
      && IS_SET(attacker->act2, ACT2_PLR_ARENA)){ 
      return true; 
   } 
   if(!IS_NPC(victim) && !IS_NPC(attacker) 
//      && IS_SET(victim->act2, ACT2_PLR_EVENT) 
//      && IS_SET(attacker->act2, ACT2_PLR_EVENT)
	){ 
      return true; 
   } 
