...
//DECLARE_DO_FUN( do_xpquest);
DECLARE_DO_FUN( do_silently);
DECLARE_DO_FUN( do_restore);
DECLARE_DO_FUN( do_boon);
...
const	struct	mob_cmd_type	mob_cmd_table	[] =
{
	...
	{"at",do_mpat,"Lets the mobile do a command at another location",
		...
		"Like imm at command"},
	{"boon",do_boon, "causes mob to boon the target as like imm boon ability",
		"mob boon <target>",
		" mob boon $n"},
	...
	/*{"xpquest", do_xpquest, "turns on double experience points",
		"mob xpquest on <ticks>"
	},*/
...
/**************************************************************************/
/*void do_xpquest( char_data *ch, char *argument )
{
    char arg[MIL];
    char arg1[MIL];
    ...
    if ( arg[0] == '\0' )
    {
        ch->println("Syntax: double <on|off>.");
        return;
    }                   
    ...
    if (!str_cmp(arg, "on"))
    {
        if ( arg1[0] == '\0' )
        {
            ch->println("You need to apply the number of ticks.");
            return;
        }

        if (double_exp)
        {
            ch->println("Double exp is already in affect!");
            return;
        }  
...
//to begin with 
        ch->println("Double exp is now in affect!");
        return;
    }
    ...
    if (!str_cmp(arg, "off"))
    {
        if (!double_exp)
        {
          ch->println("Double exp is not on please turn it on first!");
            return;
        }

        double_exp = false;
        global_exp = 0; 
         info_broadcast( ch, "%s has removed double experience!",ch->name );
        ch->println( "You have turned off double exp!");
        return;
    }
} */                
