...
SPRESULT spell_armor( int sn, int level, char_data *ch, void *vo, int )
{
	...
	{
		if (victim == ch)
			ch->println( "You are already strengthened against attack." );
		else
			act("$N is already strengthened against attack.",ch,NULL,victim,TO_CHAR);
		return HALF_MANA;
	}
	...
	victim->println( "You feel strengthened against attack." );

	if ( ch != victim )
		act("$N is strengthened against attack.",ch,NULL,victim,TO_CHAR);
    return FULL_MANA;
}
...
SPRESULT spell_bless( int sn, int level, char_data *ch, void *vo, int target)
{
	...
	if(is_affected( victim, gsn_curse )){
		if( check_dispel( level, victim, gsn_curse )){
			act( "Your blessing undoes a curse from the gods for $N.", ch, NULL, victim, TO_CHAR );
			return FULL_MANA;
        }else{
	...
	if(is_affected( victim, sn )){
		if (victim == ch){
			ch->println( "You are already blessed by the gods." );
		}else{
	...
	affect_to_char( victim, &af );
	victim->println( "You feel the favor of the gods upon you." );

	if ( ch != victim )
		act("You grant $N the favor of the gods.",ch,NULL,victim,TO_CHAR);
	return FULL_MANA;
}
...
SPRESULT spell_cancellation( int , int level, char_data *ch, void *vo,int  )
{
	...
	{
		act("$n is no longer under the protection of sanctuary.",victim,NULL,NULL,TO_ROOM);
		found = true;
	}
	if (check_dispel(level,victim, gsn_shield ))
	{
		act("$n is no longer protected by the the gods.",victim,NULL,NULL,TO_ROOM);
		found = true;
	}
...
SPRESULT spell_charm_person( int sn, int level, char_data *ch, void *vo,int  )
{
	...
	act( "You find yourself enthralled with $n.", ch, NULL, victim, TO_VICT );

	if ( ch != victim ) {
		act("$N appears to be enthralled with you.",ch,NULL,victim,TO_CHAR);
	}
...
SPRESULT spell_cure_blindness(int ,int level,char_data *ch,void *vo,int )
{
	...
	if (check_dispel(level,victim,gsn_blindness))
	{
		victim->println( "Your vision returns, thank the gods!" );
		act("$n is no longer blinded.",victim,NULL,NULL,TO_ROOM);
	}else{
		ch->println("Treatment failed.");
	}
...
SPRESULT spell_cure_disease( int , int level, char_data *ch,void *vo,int )
{
	...
	{
		victim->println( "Your disease is cured." );
		act("$n looks relieved as $s disease is cured.",victim,NULL,NULL,TO_ROOM);
	}
	else
		ch->println("Treatment failed.");
...
SPRESULT spell_curse( int sn, int level, char_data *ch, void *vo,int target )
{
	...
	if (target == TARGET_OBJ)
	{
		...
		if (IS_OBJ_STAT(obj,OBJEXTRA_BLESS))
		{
			...
			{
				act("The good nature of $p is too powerful for you to overcome.",ch,obj,NULL,TO_CHAR);
				return FULL_MANA;
			}
		}
		...
		act("$p has been cursed by the gods.",ch,obj,NULL,TO_ALL);
		return FULL_MANA;
	}
	...
	if( is_affected( victim, gsn_bless ))
		if( check_dispel( level, victim, gsn_bless ))
		{
			act( "Your efforts unravel a spell on $N.", ch, NULL, victim, TO_CHAR );
			return FULL_MANA;
		}
		else
		{
	...
	if(saves_spell(level,victim,DAMTYPE(sn))){
		ch->println( "Your curse doesnt appear to have much affect." );
		return FULL_MANA;
	}
			
	if(HAS_CLASSFLAG(victim, CLASSFLAG_CURSE_IMMUNITY)) // fails if paladin
	{
		ch->printlnf( "Your curse doesnt appear to have any affect on %s.", PERS(victim, ch) );
		return FULL_MANA;
	}
	...
	victim->println( "You feel as though you are cursed by the gods." );
	if ( ch != victim )
	{
		act("$N looks rather worried.",ch,NULL,victim,TO_CHAR);
...
SPRESULT spell_dispel_good( int sn, int level, char_data *ch, void *vo,int  )
{
	...
	if ( IS_EVIL(victim) )
	{
		act( "$N is protected by $S own evil nature.", ch, NULL, victim, TO_ROOM );
		return FULL_MANA;
	}
...
SPRESULT spell_dispel_magic( int sn , int level, char_data *ch, void *vo,int  )
{
	...
	if (check_dispel(level,victim,gsn_shield))
	{
		act("$n is no longer protected by the gods.",victim,NULL,NULL,TO_ROOM);
		found = true;
	}
	if (check_dispel(level,victim,gsn_sleep))
		found = true;
...
SPRESULT spell_earthquake( int sn, int level, char_data *ch, void *,int  )
{
	...
	ch->println( "`cThe seas swell as a violent storm approaches!`x" );
	act( "`c$n calls a violent storm, swelling the seas.`x", ch, NULL, NULL, TO_ROOM );
    ...
    for ( vch = ch->in_room->people; vch; vch = vch_next )
	{
		...
		if ( vch->in_room->area == ch->in_room->area ){
			vch->println( "`cThe waves of the sea stike harshly as a typhoon rages.`x" );
		}
	}
    return FULL_MANA;
}
...
SPRESULT spell_identify( int, int, char_data *ch, void *vo, int  )
{
	...
	ch->printf( "Object '%s' is type %s,\r\n"
		...
		obj->cost,
		obj->level );
	/*add in "extra3 flags %s.\r\n" and extra3_bit_name( obj->extra3_flags ),*/
...
SPRESULT spell_plague( int sn, int level, char_data *ch, void *vo, int )
{
	...
	if ( IS_ICIMMORTAL(victim)){
		act( "$n doesn't appear to be even slightly affected by your powers.", victim, NULL, NULL, TO_ROOM );
		return FULL_MANA;
	}
...
SPRESULT spell_refresh( int , int level, char_data *ch, void *vo,int  )
{
	...
	if ( ch != victim )
		ch->println( "They appear more refreshed." );
	return FULL_MANA;
}
...
SPRESULT spell_remove_curse( int , int level, char_data *ch, void *vo,int target)
{
	...
	{
		ch->printf( "It doesn't appear your power has had any affect.\r\n" );
	}
	return FULL_MANA;
}
...
SPRESULT spell_sanctuary( int sn, int level, char_data *ch, void *vo,int )
{
	...
	{
		if (victim == ch)
			ch->printf( "You are already protected by sanctuary.\r\n" );
		else
			act("$N is already protected by sanctuary.",ch,NULL,victim,TO_CHAR);
		return HALF_MANA;
	}
	...
	affect_to_char( victim, &af );
	act( "$n is protected by a sanctuary as per the will of the gods.", victim, NULL, NULL, TO_ROOM );
	victim->printf( "You are now protected by sanctuary.\r\n" );
	return FULL_MANA;
}
...
SPRESULT spell_shield( int sn, int level, char_data *ch, void *vo,int )
{
	...
	if ( is_affected( victim, sn ) )
	{
		if (victim == ch)
			...
			act("$N is already shielded from harm by the gods.",ch,NULL,victim,TO_CHAR);
		return HALF_MANA;
	}
	...
	af.bitvector = 0;
	affect_to_char( victim, &af );
	act( "$n is shielded from harm by the gods.", victim, NULL, NULL, TO_ROOM );
	victim->printf( "You are shielded from harm by the gods.\r\n" );
	return FULL_MANA;
}
...
SPRESULT spell_weaken( int sn, int level, char_data *ch, void *vo,int )
{
	...
	if( is_affected( victim, gsn_giant_strength ))
		if( check_dispel( level, victim, gsn_giant_strength ))
		{
			act( "Your powers unravel a blessing of strength on $N.", ch, NULL, victim, TO_CHAR );
			return FULL_MANA;
		}
		else
		{
	...
	if (saves_spell( level, victim,DAMTYPE(sn)) )
	{
		ch->printf( "Your powers dont appear to have much affect.\r\n" );
		return FULL_MANA;
	}
