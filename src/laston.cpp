...
void laston_update_node(laston_data *node, char_data *ch)
{
    ...
    node->charfame = ch->pcdata->fame;
    node->charfame = ch->pcdata->military_points;
    node->charfame = ch->pcdata->clout;
    node->charfame = ch->pcdata->divinity;
    node->xp = ch->exp;
...
void resort_top_roleplayers()
{
    ...
    for (node= laston_list; node; node=node->next)
    {
		if (node->rps>=200  // nonimms on in the last 8 weeks
			&& (node->level[node->index]<LEVEL_IMMORTAL)
...
void do_toprp(char_data *ch, char *)
{
    ...
    output= new_buf();
        ch->println("`#`?    Top Role-players:`x");

    for (node= laston_rpers_list; node; node=node->next_rper)
    {
//		sprintf(buf, "`?%-13s%6d`x", node->name, node->rps);
		sprintf(buf, "`?%3d %-13s%6d`=?\n\r", count, node->name, 
node->rps);
	    add_buf(output,buf);
...
/***************************************************************************
* This code stolen and modified from do_laston_level_one_pwipe by Zeus     *
* of Athens.								   *
***************************************************************************/

void do_playerwipe(char_data *ch, char *argument)
{
	int dllevel;
	int dage;
	char arg1[MSL];
	...
	//These lines copied from do_laston_level_one_pwipe
	...
	//NPC's don't need to know how to pwipe
	if(IS_NPC(ch))
	{do_huh(ch,"");return;}

	//Make sure both a level and age to pwipe for is supplied
	//if not, spit out the syntax to admonish the guilty one
        dllevel=atoi(arg1);
	//dage=atoi(arg2)*(60*60*24);
	dulevel=atoi(arg2);
	dage=atoi(arg3)*(60*60*24);
	...
	{
	//ch->printlnf("Syntax:PLAYERWIPE <lower level> <age>");
	ch->printlnf("Syntax:PLAYERWIPE <lower level> <upper level> <age>");
	ch->printlnf("`#`YNote 1`^:age is the number of days that you wish to delete at!");
	ch->printlnf("`#`YNote 2`^:If you only want to delete for one level indicate that\n\rlevel for both lower and upper level.");
	return;
	}
	
	//Imm Pfiles must be deleted manually by an IMP from the Shell

	if(dllevel >= 92 || dulevel >= 92)
	//if(dllevel >= 92)
	{
	ch->printlnf("You can not use this command to delete Immortal Player Files.");
	ch->printlnf("They must be deleted manually.");
	return;
	}

	//Everything smooth so far? Then lets ditch those rotting pfiles!
	//stolen and modified from do_laston_level_one_pwipe
	for (node= laston_list; node; node=node_next)
	{	
	node_next=node->next;	
	ni= node->index;		
        //if (node->level[ni]==dllevel && !node->deleted_date && ((node->off[ni]+(dage)) < current_time ))
	if (node->level[ni]>=dllevel && node->level[ni]<=dulevel && !node->deleted_date && ((node->off[ni]+(dage)) < current_time ))
		{
		count++;
		ch->printlnf("%3d) LASTON - Deleting old file %s ", count, node->name);		
		// find the pfile and remove it		
		PFILE_TYPE pt=find_pfiletype(node->name);
			if(pt==PFILE_NONE)
			{			
			ch->printlnf("ERROR: a pfile for %s wasnt found!", node->name);
			logf("do_playerwipe(): ERROR - a pfile for %s wasnt found!", node->name);
			continue;
			}					
			if(pt==PFILE_MULTIPLE)
			{			
	...
	sprintf( buf, pfilename(node->name, pt));
	logf("do_playerwipe():[%3d] removing level %d pfile %s from %s", dage, count, node->name, buf);
	unlink(buf);
	laston_id_delete(node->id); // remove them from the laston list
 		}
	} // next node in list
