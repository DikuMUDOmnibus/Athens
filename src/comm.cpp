		...
		if( c->showstr_point ){
			if(c->mxp_enabled){
				write_to_buffer( c, FORMATF("`#`=\xaa%s`^\r\n", 
					mxp_tagify("<send href=\"continue\">`c[Hit Return to continue]`x</send>")),
					0 );
			}else{
				write_to_buffer( c, "`#`=\xaa""`c[Hit Return to continue]`x`^\r\n", 0 );
			}
		}else{
		...
		case 'd': // misc data about your character status - Balo & Kal May 2002 
			{
				...
				if( ch->iwizi){
					strcat( buf2, FORMATF(" `#(`Ciw`c%d`&)", ch->iwizi));
				}
				if( ch->owizi){
					strcat( buf2, FORMATF(" `#(`Cow`c%d`&)", ch->owizi));
				}
	...
	for ( ch = char_list; ch != NULL; ch = ch->next )
	{
		...
		&&   !str_cmp( c->character->name, ch->name ) )
		{
			...
			else // ditch the duplicates that were just loaded
			{
				...
				ch->println("Reconnecting.  Use `Rreplay`c, `Rreplayroom`c, and `Rreplaychan`c to see missed events.");
