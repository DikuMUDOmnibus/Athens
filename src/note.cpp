...
void do_unread(char_data *ch, char *)
{
    ...
    {
		ch->printlnf( "`CNews`c:    There %s %d new message%s.`x",
			count > 1 ? "are" : "is",count, count > 1 ? "s" : "");
	}
	else 
		ch->println( "`CNews`c:    There are no new messages.`x" );

	if ((count = count_spool(ch,changes_list)) > 0)
	{
		ch->printlnf( "`CChange`cs: There %s %d new message%s.`x",
			count > 1 ? "are" : "is", count, count > 1 ? "s" : "");
	}
	else
		ch->println( "`CChange`cs: There are no new messages.`x" );

	if ((count = count_spool(ch,note_list)) > 0)
	{
		ch->printlnf( "`CNote`cs:   There %s %d new message%s.`x",
			count > 1 ? "are" : "is", count, count > 1 ? "s" : "");
	}
	else
		ch->println( "`CNote`cs:   There are no new messages.`x" );

	if ((count = count_spool(ch,idea_list)) > 0)
	{
		ch->printlnf( "`CIdea`cs:   There %s %d new message%s.`x",
			count > 1 ? "are" : "is", count, count  > 1 ? "s" : "");
	}
	else
		ch->println( "`CIdea`cs:   There are no new messages.`x" );

	if (IS_TRUSTED(ch,ANGEL) && (count = count_spool(ch,penalty_list)) > 0)
	{
		ch->printlnf( "`CPenalty: There %s %d %s.`x",
			count > 1 ? "are" : "is", count, count > 1 ? "penalties" : "penalty");
	}
	...
	if (IS_ADMIN(ch))
	{
		...
		{
			ch->printlnf( "`CAnote`cs:  There %s %d new message%s.`x",
				count > 1 ? "are" : "is", count, count > 1 ? "s" : "");
		}
		else
			ch->println( "`CAnote`cs:  There are no new messages.`x" );
	}
	...
	if (IS_IMMORTAL(ch))
	{
		...
		{
			ch->printlnf( "`CInote`cs:  There %s %d new message%s.`x",
				count > 1 ? "are" : "is", count, count  > 1 ? "s" : "");
		}
		else
			ch->println( "`CInote`cs:  There are no new messages.`x" );
	}
	...
	if (HAS_CONFIG(ch, CONFIG_SHOWMISC) && !HAS_CONFIG(ch, CONFIG_NOMISC))
	{
		...
		{
			ch->printlnf( "`CMisc`c: There %s %d new message%s.`x",
				count > 1 ? "are" : "is", count, count > 1 ? "s" : "");
		}
		else
			ch->println( "`CMisc`c: There are no new messages.`x" );
	}

	if ( TRUE_CH(ch)->level>=96){	//////// temp!! til we go live
		if ((count = count_spool(ch,snote_list)) > 0)
		{
			ch->printlnf( "`CSnote`cs:  There %s %d new message%s`x",
				count > 1 ? "are" : "is", count, count > 1 ? "s" : "");
		}
		else
			ch->println( "`CSnote`cs:  There are no new messages.`x" );
	}

	if (( count = count_spool(ch,pknote_list)) > 0)
	{
		ch->printlnf("`CPKnote`cs:  There %s %d new message%s.`x",
			count > 1 ? "are" : "is", count, count > 1 ? "s" : "");
	}
    if (double_exp)
	{
