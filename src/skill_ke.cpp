...
void forage_food( char_data *ch )
{
	...
	if ( ch->in_room->sector_type == SECT_INSIDE
		|| ch->in_room->sector_type == SECT_CITY
                || ch->in_room->sector_type == SECT_HOME
	...
	if (( pObjIndex = get_obj_index( OBJ_VNUM_EGG )) == NULL ) {
	...
	case SECT_FIELD:
		pObj->description = str_dup( "You see a bunch of berries here." );
		pObj->short_descr = str_dup( "a bunch of berries" );
		pObj->name		  = str_dup( "berries bunch" );
		break;
	case SECT_FOREST:
		pObj->description = str_dup( "You see a handful of almonds." );
		pObj->short_descr = str_dup( "a handful of almonds" );
		pObj->name		  = str_dup( "almonds" );
		break;
	case SECT_HILLS:
...
/**************************************************************************/
/*  I replaced this with a more workable piece of code in mix.cpp - Hera
...
void do_dig( char_data *ch,	char * )
{
	...
	if ( found ) {
		...
		act( "$n has unearthed something.", ch, NULL, NULL, TO_ROOM );
                ...
                act( "$n finds nothing through digging.", ch, NULL, NULL, TO_ROOM );
 	}
}
...
void do_herblore( char_data *ch, char *argument )
{
	...
	{
	case (1):
		ch->printlnf( "A %s only grows %s.",
			h->name,
			sector_desc[h->sector].name );
		break;
	case (2):
		if ( !h->continent)	{
			ch->printlnf( "A %s is prolific, and can be found in any region of the known world.",
				h->name);
		}else{
			ch->printlnf( "A %s is indigenous to %s.",
				h->name,
				h->continent->name );
		}
		break;
	case (3):
		if ( h->month == -1 )
		{
			ch->printlnf( "A %s is a perennial plant.",
				h->name );
		}
		else
		{
			ch->printlnf( "A %s only grows in the month of %s.",
				h->name,
				month_name[h->month] );
		}
		break;
	case (4):
		...
		{
			ch->printlnf( "A %s only grows during the season of %s.",
				h->name,
	...
	case (5):
		...
		{
			ch->printlnf( "A %s may be harvested successfully at any time of the day or night.",
				h->name );
		}
		else
		{
			ch->printlnf( "A %s can only be successfully harvested %s.",
				h->name,
	...
	act( "$n studies a $p.", ch, herb, NULL, TO_ROOM );
