...
const struct flag_type game_settings_flags[] =
{
	...
	{ "athensflag_athens",				GAMESET2_ATHENSFLAG_ATHENS, true},
	{ "athensflag_megara",				GAMESET2_ATHENSFLAG_MEGARA, true},
	{ "megaraflag_athens",				GAMESET2_MEGARAFLAG_ATHENS, true},
	{ "megaraflag_megara",				GAMESET2_MEGARAFLAG_MEGARA, true},
	{ "assassins",				GAMESET2_ASSASSINS, true},	
	{ "applebet",				GAMESET2_APPLEBET, true},	
	{ "discord",				GAMESET3_DISCORD, true},	
	{ NULL,0,0}
};
/**************************************************************************/
const struct flag_type game_settings_flags2[] =
{
	...
	{ "autonote_immpkill_to_admin",	GAMESET2_AUTONOTE_IMMPKILLS_TO_ADMIN, false},
...
const struct flag_type game_settings_flags5[] =
{
	...
	{ "dedicated_pkill_style_mud",		GAMESET5_DEDICATED_PKILL_STYLE_MUD, true},	
...
GIO_INT_WITH_DEFAULT(global_scale_hitpoints_regen_rate, 115)
...
GIO_INT_WITH_DEFAULT(obj_vnum_egg,                             379)
GIO_INT_WITH_DEFAULT(obj_vnum_gemstone,			100)
GIO_INT_WITH_DEFAULT(obj_vnum_fishcatch,			101)
...
GIO_INT_WITH_DEFAULT(global_xp_scale_value,		150) // Kal, Oct03
...
gameset_value_type gameset_value[]={
	...
	{ GSVC_OBJECT, GSBYTE_OFFSET(obj_vnum_rose), 			"rose", "obj_vnum_rose"},
        { GSVC_OBJECT, GSBYTE_OFFSET(obj_vnum_egg),                    "egg", "obj_vnum_egg"},
...
/**************************************************************************/
// Hera - April 2005
void do_athensflag(char_data *ch, char *argument)
{	
	...
	if(IS_NULLSTR(argument)){
		if(GAMESETTING(GAMESET2_ATHENSFLAG_ATHENS)){
			ch->wrapln("`cThe Athens City Flag is currently set in `CAthens`c.`x");}
		if(GAMESETTING(GAMESET2_ATHENSFLAG_MEGARA)){
			ch->wrapln("`cThe Athens City Flag is currently set in `CMegara`c.`x");}
		return;
	}

	if(!str_cmp("megara", argument))
	{
			ch->println("`cThe Athens City Flag is now set in `CMegara`c.`x");
			REMOVE_BIT(game_settings->flags, GAMESET2_ATHENSFLAG_ATHENS);
			SET_BIT(game_settings->flags, GAMESET2_ATHENSFLAG_MEGARA);
			do_save_gamesettings(NULL, ""); 
			return;
	}
	if(!str_cmp("athens", argument))
	{
			ch->println("`cThe Athens City Flag is now set as in `CAthens`c.`x");
			SET_BIT(game_settings->flags, GAMESET2_ATHENSFLAG_ATHENS);
			REMOVE_BIT(game_settings->flags, GAMESET2_ATHENSFLAG_MEGARA);
			do_save_gamesettings(NULL, ""); 
			return;
	}
	ch->printlnf("`cAthensflag parameter `C'%s'`c incorrect.`x", argument);
	do_athensflag(ch,"");
}
/**************************************************************************/
// Hera - April 2005
void do_megaraflag(char_data *ch, char *argument)
{	
    if(!IS_NPC(ch) && ch->level < 97 )
    {do_huh(ch,"");return;}  
	...
	if(IS_NULLSTR(argument)){
		if(GAMESETTING(GAMESET2_MEGARAFLAG_ATHENS)){
			ch->wrapln("`cThe Megara City Flag is currently set in `CAthens`c.`x");}
		if(GAMESETTING(GAMESET2_MEGARAFLAG_MEGARA)){
			ch->wrapln("`cThe Megara City Flag is currently set in `CMegara`c.`x");}
		return;
	}

	if(!str_cmp("megara", argument))
	{
			ch->println("`cThe Megara City Flag is now set in `CMegara`c.`x");
			REMOVE_BIT(game_settings->flags, GAMESET2_MEGARAFLAG_ATHENS);
			SET_BIT(game_settings->flags, GAMESET2_MEGARAFLAG_MEGARA);
			do_save_gamesettings(NULL, ""); 
			return;
	}
	if(!str_cmp("athens", argument))
	{
			ch->println("`cThe Megara City Flag is now set in `CAthens`c.`x");
			SET_BIT(game_settings->flags, GAMESET2_MEGARAFLAG_ATHENS);
			REMOVE_BIT(game_settings->flags, GAMESET2_MEGARAFLAG_MEGARA);
			do_save_gamesettings(NULL, ""); 
			return;
	}
	ch->printlnf("`cMegaraflag parameter `C'%s'`c incorrect.`x", argument);
	do_athensflag(ch,"");
}
/**************************************************************************/
...
/**************************************************************************/
// Hera - April 2005
void do_assassins(char_data *ch, char *argument)
{	
    connection_data *d;

if(!str_cmp(ch->name, "nemesis") || !str_cmp(ch->name, "hera") || !str_cmp(ch->name, "eris")
	|| !str_cmp(ch->name, "zeus"))
{

	if(IS_NULLSTR(argument)){
		if(GAMESETTING(GAMESET2_ASSASSINS)){
			ch->wrapln("`cYour assassins are currently for hire.`x");}
		if(!GAMESETTING(GAMESET2_ASSASSINS)){
			ch->wrapln("`cYour assassins are currently NOT for hire.`x");}
	...
	{
			ch->println("`cYour assassins are now available for hire.`x");
			SET_BIT(game_settings->flags, GAMESET2_ASSASSINS);
			do_save_gamesettings(NULL, ""); 
    			for ( d = connection_list; d; d = d->next )
    			{
				if ( d->connected_state == CON_PLAYING )
			{
				d->character->printlnf( "`cThe `Cassassins `cof `CNemesis`c are now available for hire!`x");
			}
	}
			return;
	}
	...
	{
			ch->println("`cYou recall your dastardly assassins for later use.`x");
			REMOVE_BIT(game_settings->flags, GAMESET2_ASSASSINS);
			do_save_gamesettings(NULL, ""); 
    			for ( d = connection_list; d; d = d->next )
    			{
				if ( d->connected_state == CON_PLAYING )
			{
				d->character->printlnf( "`cThe `Cassassins `cof `CNemesis`c are no longer available.`x");
			}
			return;
	}
	ch->printlnf("`cASSASSINS `C'%s'`c is not correct.  Try ON or OFF.`x", argument);
	do_athensflag(ch,"");
   	return;
 	}else{
   	ch->println( "`cRevenge is to be served cold only by Nemesis.`x" );
   	return;
   	}
}
}
...
if(!str_cmp(ch->name, "eris") || !str_cmp(ch->name, "hera") || !str_cmp(ch->name, "zeus"))
{

	if(IS_NULLSTR(argument)){
		if(GAMESETTING(GAMESET3_DISCORD)){
			ch->wrapln("`cDiscordia is currently open!`x");
                        ch->wrapln("`cSyntax: DISCORD OPEN/CLOSE`x");}
		if(!GAMESETTING(GAMESET3_DISCORD)){
			ch->wrapln("`cDiscordia is closed.`x");
                        ch->wrapln("`cSyntax: DISCORD OPEN/CLOSE`x");}
		return;
	}

	if(!str_cmp("open", argument))
	{
			ch->println("`cYou have opened your maze of discord.`x");
			SET_BIT(game_settings->flags, GAMESET3_DISCORD);
			do_save_gamesettings(NULL, ""); 
    			for ( d = connection_list; d; d = d->next )
    			{
				if ( d->connected_state == CON_PLAYING )
			{
				d->character->printlnf( "`CEris`c declares the `CMaze`c of `CDiscordia`c open!`x");
				d->character->printlnf( "`cType JOIN DISCORD to enter the maze.`x");
			}
	}
			return;
	}
	if(!str_cmp("close", argument))
	{
			ch->println("`cYou close entrance into your maze of discord.`x");
			REMOVE_BIT(game_settings->flags, GAMESET3_DISCORD);
			do_save_gamesettings(NULL, ""); 
    			for ( d = connection_list; d; d = d->next )
    			{
				if ( d->connected_state == CON_PLAYING )
			{
				d->character->printlnf( "`CEris`c has closed the `CMaze`c of `CDiscordia`c.`x");
			}
			return;
	}
	ch->printlnf("`cDiscord `C'%s'`c is not correct.  Try OPEN or CLOSE.`x", argument);
	do_athensflag(ch,"");
   	return;
 	}else{
   	ch->println( "`cDiscord and Strife are the toils given by Eris.`x" );
   	return;
   	}
}
}
/**************************************************************************/
// Hera - April 2005
void do_apple(char_data *ch, char *argument)
{	
    connection_data *d;

	if(IS_NULLSTR(argument))
	{
		if(GAMESETTING(GAMESET2_APPLEBET)){
			ch->wrapln("`cYour apple is midair, able to be wagered on.`x");}
		if(!GAMESETTING(GAMESET2_APPLEBET)){
			ch->wrapln("`cYour apple is not airborne.`x");}

		return;
	}

	if(!str_cmp("throw", argument))
	{
		for ( d = connection_list; d; d = d->next )
		{
			if ( d->connected_state == CON_PLAYING )
			{
			d->character->printlnf( "`cEris throws a golden apple towards Aphrodite, Athena and Hera!`x");
			d->character->printlnf( "`M[`GAPPLE`M]`G:`g Wagers on the winner will now be taken!`x");
			return;
			}
		}
		ch->println("`cYou throw a shiney, irrestiable apple at three goddesses.`x");
		SET_BIT(game_settings->flags, GAMESET2_APPLEBET);
		do_save_gamesettings(NULL, ""); 
		return;
	}
/*
	if(!str_cmp("catch", argument))
	{
            //Apple betting system, pays the winners 
		REMOVE_BIT(game_settings->flags, GAMESET2_APPLEBET);
		do_save_gamesettings(NULL, ""); 
		return;
		...
		int value;                   
                ...
                if (IS_SET(wch->act2,ACT2_PLR_WAGER)) 
                  { 
   			appleevent = dice(1,3); 
   			switch(appleevent) 
				{
				case 1:
					if( !str_cmp( wch->pcdata->wager, "hera" ) )
					{
                     		value = wch->apot * 2; 
                     		wch->gold += value; 
                     		wch->apot = 0; 
                     		wch->printlnf("`M[`GAPPLE`M]`G:`g Hera caught the apple! You receive %d gold.`x\n\r", value); 
                     		wch->pcdata->wager = NULL; 
                     		REMOVE_BIT(wch->act2, ACT2_PLR_WAGER);
			     		return; 
                     		}else{                     { 
                     		wch->printlnf("`M[`GAPPLE`M]`G:`g You lost the %d gold you wager!`x\n\r", wch->pot); 
                     		wch->apot = 0; 
                     		wch->pcdata->wager = NULL; 
                     		REMOVE_BIT(wch->act2, ACT2_PLR_WAGER); 
		    			break; 
				...
				case 2:
					if( !str_cmp( wch->pcdata->wager, "aphrodite" ) )
                     		{ 
                     		value = wch->apot * 2; 
                     		wch->gold += value; 
                     		wch->apot = 0; 
                     		wch->printlnf("`M[`GAPPLE`M]`G:`g Aphrodite caught the apple! You receive %d gold.`x\n\r", value); 
                     		wch->pcdata->wager = NULL; 
                     		REMOVE_BIT(wch->act2, ACT2_PLR_WAGER);
		     			return; 
                     		}else{                     { 
				...
				case 3:
					if( !str_cmp( wch->pcdata->wager, "athena" ) )
                     		{ 
                     		value = wch->apot * 2; 
                     		wch->gold += value; 
                     		wch->apot = 0; 
                     		wch->printlnf("`M[`GAPPLE`M]`G:`g Athena caught the apple! You receive %d gold.`x\n\r", value); 
                     		wch->pcdata->wager = NULL; 
                     		REMOVE_BIT(wch->act2, ACT2_PLR_WAGER);
		     			return; 
                     		}else{                     { 
