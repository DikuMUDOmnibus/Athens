...
void save_object_NAFF( FILE *fp, OBJ_INDEX_DATA *pObjIndex )
{
	...
	//fwrite_wordflag( objextra3_flags,       pObjIndex->extra3_flags,"Extra3   ", fp);
	...
	for ( ; ; )
	{
		...
		for ( ; ; )
		{
			...
			case 'E':
				...
				KEY( "Extra2",		pObjIndex->extra2_flags,		fread_wordflag( 
objextra2_flags, fp ));
				/*KEY( "Extra3",          pObjIndex->extra3_flags,                fread_wordflag(
objextra3_flags, fp ));*/
				if ( !str_cmp( word, "ExtraDesc" ))
				{
					EXTRA_DESCR_DATA *ed=new_extra_descr();
...
/*
	if(IS_SET(wear_flags, T)){
		SET_BIT(pObjIndex->wear_flags, OBJWEAR_HAIR);
	}
	if(IS_SET(wear_flags, T)){
		SET_BIT(pObjIndex->wear_flags, OBJWEAR_ARMOR);
	}
*/
	// extra flags
	if(IS_SET(pObjIndex->extra_flags, bb)){
