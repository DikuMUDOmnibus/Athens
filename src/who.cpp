...
struct	who_format_type who_formatter [] =
{
	...
	{ "default",	   	        NULL,				false		},
        // spacer - if you select default you get the mud whoformat setting
	{ "athens",			whoformat_athens,		false		},
...
#define WHOHEADER_MORTALS "`C-============ `GMORTALS `C============-`x\r\n"
#define WHOHEADER_IMMORTALS "`C-======== `YGODS OF OLYMPUS `C========-`x\r\n"
#define WHOHEADER_PKILL_PORT "`r-`R=`r=`R=`r=`R=`r=`R=`r=`R=`r=`R=`r= `R P K I L L   P O R T  `r=`R=`r=`R=`r=`R=`r=`R=`r=`R=`r-`x\r\n"
    ...
    if (IS_NEWBIE(ch)){
        add_buf(output,"NSP(newbie support) are happy to answer questions.\r\n");
    }
