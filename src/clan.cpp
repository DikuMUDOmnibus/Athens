...
void do_addclan( char_data *ch, char *argument )
{
    ...
    {
		ch->println( "Addclan can only be used by Admin." );
        return;
    }
	...
	{
        ch->println( "They are now in any army." );
        victim->println( "You are now not in any army!" );
		victim->clan	= NULL;
		victim->clanrank = 0;
        return;
	}
	...
	{
        ch->printlnf( "No such army '%s' exists.", arg2 );
        return;
	}
...
void do_clanlist(char_data *ch, char *)
{
    ...
    ch->print( "`c-`CArmy Name`c==================================`CNoteName`c========`CRecall`c");

	if(IS_ADMIN(ch)){
		ch->println("`c====`CFunds`c-`^");
	}else{
		ch->println("-`^");
	}
...
void clan_show_ranks(char_data *ch, char_data *victim, CClanType *pClan)
{
	if(pClan){
		...
		if(victim){
			ch->printlnf( "%s's rank in this army is: `#%s%s`^",
				PERS(victim, ch),
				pClan->color_str(), 
		...
		if ( ch->clan == pClan){
			ch->printlnf( "Your rank in this army is: `#%s%s`^",
				pClan->color_str(), 
...
{	
	...
	if(!IS_IMMORTAL(ch) && !ch->clan){
		ch->println("You serve no army, you have no use for this command.");
		return;
	}
	...
	if(!victim){
		...
		if(IS_IMMORTAL(ch)){ // imms can also specify the clan name
			...
			if(!pClan){
				ch->printlnf( "No such army or player '%s' is currently in the game.", argument );
				clan_show_clanrank_syntax(ch);
				return;
			}
		...
		ch->printlnf( "No such army member '%s' is currently in the game.", argument );	
		return;
	}
	...
	if(IS_IMMORTAL(ch)){
		if(!victim->clan){
			ch->printlnf("%s is not in an army.", PERS(victim, ch));
			clan_show_clanrank_syntax(ch);
			return;
		}
	}else{
		if(!victim->clan || victim->clan!=ch->clan){
			ch->printlnf("%s is not in the same army as you.", PERS(victim, ch));
			clan_show_clanrank_syntax(ch);
			return;
		}
...
void do_add( char_data *ch, char *argument )
{
    ...
    argument = one_argument( argument, arg1 );
    if( ch->clan  == NULL ) {
        ch->println( "You are not in an army yourself!" );
        return;
    }
    ...
    if( victim->clan  ) {
        ch->printlnf( "%s is already in an army.", PERS(victim, ch) );
        return;
    }

    if( victim->seeks != ch->clan ) {
        ch->wraplnf("They are not seeking to join your army."
			"They have been told you invited them to join, and to seek your army if interested"
			"- try again shortly.");

        victim->wraplnf("%s invited you to become a member of an "
            "army, '%s'.  You weren't seeking that "
            "army, type 'seek %s' if you want to join, and when they invite "
    ...
    ch->printlnf( "They are now a %s of your army.",
		victim->clan->clan_rank_title(victim->clanrank));
	victim->printlnf( "You are now a %s of army %s.",
		victim->clan->clan_rank_title(victim->clanrank),
		victim->clan->name());
...
void do_demote( char_data *ch, char *argument )
{
    ...
    if( victim->clan  != ch->clan  ) {
        ch->println( "They are not in your army." );
        return;
    }
...
void do_outcast( char_data *ch, char *argument )
{
    ...
    ch->printlnf( "They are no longer a %s of your army.",
		victim->clan->clan_rank_title(victim->clanrank) );
 	victim->printlnf( "You are no longer a %s of army %s.",
		victim->clan->clan_rank_title(victim->clanrank),
		victim->clan->name());
...
void do_promote( char_data *ch, char *argument )
{
	...
	if(victim->clanrank==0){
        ch->println( "They are already the highest rank in your army." );
        return;
	}
    ...
    ch->printlnf( "They are now a %s of your army.",
		ch->clan->clan_rank_title(victim->clanrank) );
	victim->printlnf( "Congratulations, you are now a %s of army %s.",
		ch->clan->clan_rank_title(victim->clanrank),
		ch->clan->name());
...
void do_seek( char_data *ch, char *argument )
{
    ...
    if ( !str_cmp(arg1,"none") ) {
		ch->seeks = NULL;
        ch->println( "You are seeking no army enlistment." );
        return;
    }

    if ( ch->clan ) {
        ch->println( "You are already in an army." );
        return;
    }

    if ( (pClan = clan_lookup(arg1)) == NULL ) {
		ch->println( "No such army exists." );
        return;
    }
	...
	ch->printlnf( "You are seeking to join %s.",
		pClan->name() );
	ch->println("(type 'seek none' to stop seeking an army.)");
...
void clan_bank( char_data *ch, char *task, char *amount)
{
	int number;
	if(IS_NPC(ch)){
		ch->println("army banking by players only sorry.");
		return;
	}
	
	if(!str_prefix(task,"balance") )
	{
		ch->printlnf( "Your army has %ld gold in its coffers.", ch->clan->m_BankFunds );
	...
	if(!str_prefix(task,"deposit"))
	{
		...
		ch->printlnf( "Deposit made.\r\nYour army now has %ld gold pieces in its coffers.",
			ch->clan->m_BankFunds );

		logf("[clanbank] %s deposited %d gold into army bank account %s - balance now %d",
			ch->name,
			number,
			ch->clan->who_name(),
			(int)ch->clan->m_BankFunds);
		...
		// the clanbank transaction trail
		append_datetimestring_to_file( CLANBANKING_FILE, 
			FORMATF("%s[%d] deposited %d gold into army bank account %s - balance now %d`x",
			TRUE_CH(ch)->name,
			ch->clanrank,
			number,
			ch->clan->who_name(),
    ...
    if(!str_prefix(task,"withdraw"))
	{
		if ( !ch->clan->m_CanWithdraw[ch->clanrank])
		{
			// the clanbank transaction trail
			append_datetimestring_to_file( CLANBANKING_FILE, 
				FORMATF("%s[%d] attempted to withdraw %d gold from army bank account %s but didn't have authority - balance is %d`x",
				TRUE_CH(ch)->name,
				ch->clanrank,
				number,
				ch->clan->who_name(),
				(int)ch->clan->m_BankFunds));
			ch->println( "You do not have the authority to withdraw money from your army." );
			return;
		}
		if( number < 6 )
		{
		...
		logf("[clanbank] %s withdrew %d gold from army bank account %s - balance now %d",
			ch->name,
			number,
			ch->clan->who_name(),
			(int)ch->clan->m_BankFunds);
		...
		// the clanbank transaction trail
		append_datetimestring_to_file( CLANBANKING_FILE, 
			FORMATF("%s[%d] withdrew %d gold from army bank account %s - balance now %d`x",
			TRUE_CH(ch)->name,
			ch->clanrank,
			number,
			ch->clan->who_name(),
	...
	ch->printlnf( "'%s' is not a valid armybank transaction.", task );
