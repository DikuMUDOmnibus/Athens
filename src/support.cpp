...
void do_checknsupport( char_data *ch, char *argument)
{
    ...
    {
        ch->println("Checknsupport can only be used by the support council or an Immortal.");
        return;
    }
...
/**************************************************************************/
void do_nspload( char_data *ch, char *argument)
{
    OBJ_DATA *obj;
    OBJ_INDEX_DATA *objindex;
    //int vnum;
	...
	if(IS_NULLSTR(arg))
	{
	ch->println("Syntax: nspload <item>");
	ch->println("Valid Items: light, vest, sleeves, cap, gloves, leggings, boots, belt, shield, or weapon.");
	return;
	}

	if(!strcmp(arg, "light"))
	{int light=OBJ_VNUM_OUTFIT_LIGHT;
        objindex=get_obj_index(light);
        if(objindex)
	{obj = create_object( objindex);
        obj->cost = 0;
        obj_to_char( obj, ch );
        ch->printlnf("You have created %s", obj->description);}
	else
	{ch->printlnf("That is not a valid option. Type OUTFIT without an argument for syntax.");}return;}

	if(!strcmp(arg, "vest"))
	{int vest=OBJ_VNUM_OUTFIT_VEST;
        objindex=get_obj_index(vest);
        if(objindex)
	{obj = create_object( objindex);
        obj->cost = 0;
        obj_to_char( obj, ch );
	...
	if(!strcmp(arg, "sleeves"))
	{int sleeves=OBJ_VNUM_OUTFIT_SLEEVES;
        objindex=get_obj_index(sleeves);
        if(objindex)
	{obj = create_object( objindex);
        obj->cost = 0;
        obj_to_char( obj, ch );
	...
	if(!strcmp(arg, "cap"))
	{int cap=OBJ_VNUM_OUTFIT_CAP;
        objindex=get_obj_index(cap);
        if(objindex)
	{obj = create_object( objindex);
        obj->cost = 0;
        obj_to_char( obj, ch );
	...
	if(!strcmp(arg,"gloves"))
	{int gloves=OBJ_VNUM_OUTFIT_GLOVES;
        objindex=get_obj_index(gloves);
        if(objindex)
        {obj = create_object( objindex);
        obj->cost = 0;
        obj_to_char( obj, ch );
	...
	if(!strcmp(arg, "leggings"))
        {int leggings=OBJ_VNUM_OUTFIT_LEGGINGS;
        objindex=get_obj_index(leggings);
        if(objindex)
	{obj = create_object( objindex);
        obj->cost = 0;
        obj_to_char( obj, ch );
	...
	if(!strcmp(arg, "boots"))
 	{int boots=OBJ_VNUM_OUTFIT_BOOTS;
        objindex=get_obj_index(boots);
        if(objindex)
	{obj = create_object( objindex);
        obj->cost = 0;
        obj_to_char( obj, ch );
	...
	if(!strcmp(arg, "belt"))
 	{int belt=OBJ_VNUM_OUTFIT_BELT;
        objindex=get_obj_index(belt);
        if(objindex){
        obj = create_object( objindex);
        obj->cost = 0;
        obj_to_char( obj, ch );
	...
	{int shield=OBJ_VNUM_OUTFIT_SHIELD;
        objindex=get_obj_index(shield);
        if(objindex)
	{obj = create_object( objindex);
        obj->cost = 0;
        obj_to_char( obj, ch );
	...
	{int weapon=OBJ_VNUM_OUTFIT_DAGGER;
        objindex=get_obj_index(weapon);
        if(objindex)
	{obj = create_object( objindex);
        obj->cost = 0;
        obj_to_char( obj, ch );
	...
	{
        ch->println("`#`RThat is not a valid option!`^");
	ch->println("Syntax: nspload <item>");
        ch->println("Valid Items: light, vest, sleeves, cap, gloves, leggings, boots, belt, shield, or weapon.");
        return;
        }
