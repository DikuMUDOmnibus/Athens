...
char *getwebToprp(int level);
char *getwebContests(int level);
char *getwebList(int level);
...
char *websvr_generate_head_and_start_body(char *title)
{
	...
	sprintf(head, 
		DOCTYPE
		"<HTML><HEAD>\n"		
		"<TITLE>%.8000s - A free to play mythological mud</TITLE>\n"
		"<link rel=\"StyleSheet\" href=\"/stat.css\" type=\"text/css\" />\n"
		"</HEAD>\n"
		"\n"
		"<BODY BGCOLOR=\"#000000\" TEXT=\"#fac886\" LINK=\"#fac886\" ALINK=\"#fac886\" VLINK=\"#fac886\">\n",
		title);
...
void processGET(web_request_data *w)
{
	...
	if (!str_cmp(w->requestedLocation,"/areas")){
		...
		w->outBodyLen=snprintf(w->outBodyBuf, WEBOUT_BODY_BUF,
			"%s" // doctype, header, and start of body tag
			"<H2><FONT COLOR=\"#fac886\">Areas within %s</FONT></H2>"
			"\n<PRE><FONT>%s"
			"</PRE>"
	...
	if (!str_cmp(w->requestedLocation,"/toprp")){
		webWhoHits++;
		w->statusCode=200;
		buf=getwebToprp(20);
		w->outBodyLen=snprintf(w->outBodyBuf, WEBOUT_BODY_BUF,
			"%s" // doctype, header, and start of body tag
			"<FONT COLOR=\"#fac886\"></FONT>"
			"\n<PRE><FONT>%s"
			"</PRE>"
			,websvr_generate_head_and_start_body("Top Rp:")
			,buf);
	}else
	if (!str_cmp(w->requestedLocation,"/contests")){
		webWhoHits++;
		w->statusCode=200;
		buf=getwebContests(20);
		w->outBodyLen=snprintf(w->outBodyBuf, WEBOUT_BODY_BUF,
			"%s" // doctype, header, and start of body tag
			"<H2><FONT COLOR=\"#fac886\">Contests within %s</FONT></H2>"
			"\n<PRE><FONT>%s"
			"</PRE>"
			,websvr_generate_head_and_start_body("Contests")
			,MUD_NAME
			,buf);
	}else
	if (!str_cmp(w->requestedLocation,"/quests")){
		webWhoHits++;
		w->statusCode=200;
		buf=getwebList(20);
		w->outBodyLen=snprintf(w->outBodyBuf, WEBOUT_BODY_BUF,
			"%s" // doctype, header, and start of body tag
			"<H2><FONT COLOR=\"#fac886\">Quests within %s</FONT></H2>"
			"\n<PRE><FONT>%s"
			"</PRE>"
			,websvr_generate_head_and_start_body("Quests")
			,MUD_NAME
			,buf);
	}else
	...
	if (!str_cmp(w->requestedLocation,"/who")){
		...
		w->outBodyLen=snprintf(w->outBodyBuf, WEBOUT_BODY_BUF,
			"%s" // doctype, header, and start of body tag
			"<H2><FONT COLOR=\"#fac886\">Players in %s</FONT></H2>"
			"\n<PRE><FONT>%s"
			"</PRE>"
	...
	if (!str_cmp(w->requestedLocation,"/mudstats")){
		...
		w->outBodyLen=snprintf(w->outBodyBuf, WEBOUT_BODY_BUF,
			"%s" // doctype, header, and start of body tag
			"<H2><FONT COLOR=\"#fac886\">Mudstats for %s</FONT></H2>"
			"\n<PRE><FONT>%s"
			"</PRE>"
	...
	if (!str_cmp(w->requestedLocation,"/gamesettings")){
		...
		w->outBodyLen=snprintf(w->outBodyBuf, WEBOUT_BODY_BUF, 
			...
			"<H2><FONT COLOR=\"#fac886\">%s GAME SETTINGS</FONT></H2>"
	...
	if (!str_cmp(w->requestedLocation,"/wizlist")){
		...
		w->outBodyLen=snprintf(w->outBodyBuf, WEBOUT_BODY_BUF, 
			"%s" // doctype, header, and start of body tag
			""
			"<FONT COLOR=\"#000000\"><CENTER>%s WIZLIST</CENTER></FONT>"
		...
		|| !str_cmp("/help",w->requestedLocation))
	{
		...
		sprintf(entry_buf2, "Help Files - %s", entry_buf);
		w->outBodyLen=snprintf(w->outBodyBuf, WEBOUT_BODY_BUF,
			"%s" // doctype, header, and start of body tag
			"<H2><FONT COLOR=\"#fac886\">%s Help Files </H2><H3>- %s</H3></FONT>"
#ifdef VALIDATE_HTML
			"<TT><FONT>%s</FONT></TT>"		
#else
			"<PRE><FONT>%s</PRE>"
	...
	if (!str_cmp(w->requestedLocation,"/classstats")){
		...
		w->outBodyLen=snprintf(w->outBodyBuf, WEBOUT_BODY_BUF, 
			"%s" // doctype, header, and start of body tag
			"<H2><FONT COLOR=\"#fac886\">%s CLASS STATS</FONT></H2>"
	...
	if (!str_cmp(w->requestedLocation,"/racestats")){
		...
		w->outBodyLen=snprintf(w->outBodyBuf, WEBOUT_BODY_BUF, 
			"%s" // doctype, header, and start of body tag
			"<H2><FONT COLOR=\"#fac886\">%s RACE STATS</FONT></H2>"
		...
		&& !str_cmp(w->requestedLocation,"/raceinfo"))
	{
		...
		w->outBodyLen=snprintf(w->outBodyBuf, WEBOUT_BODY_BUF, 
			"%s" // doctype, header, and start of body tag
			"<H2><FONT COLOR=\"#fac886\">%s Race Info</FONT></H2>"
	...
	if (!str_cmp(w->requestedLocation,"/alignstats")){
		...
		w->outBodyLen=snprintf(w->outBodyBuf, WEBOUT_BODY_BUF, 
			"%s" // doctype, header, and start of body tag
			"<H2><FONT COLOR=\"#fac886\">%s ALIGNMENT MATRIX</FONT></H2>"
...
,buf);
	...
	if (!str_cmp(w->requestedLocation,"/")){
		...
		w->outBodyLen=snprintf(w->outBodyBuf, WEBOUT_BODY_BUF,
		"%s" // doctype, header, and start of body tag
"<H2 ALIGN=\"CENTER\">%s - A Free to Play Mythological Mud</FONT></H2>"
/*
...
*/
"<P ALIGN=\"CENTER\"><A HREF=\"http://athens.boue.ca/\">Go to the official Athens website</A></P>"
"</P></BODY>"
"</HTML>"
